#!/usr/bin/env node
/**
 * FOOD-10D — Editorial relationship mapper for nut-seed ontology.
 ...
 *
 * Run: node scripts/map-nut-seed-editorial-relationships-10d.mjs
 */
...
import { EDITORIAL_CURATED, FORWARD_REFERENCE_IDS } from "./nut-seed-editorial-seed-10d.js";
...
const OUTPUT_PATH = path.join(RUNTIME_DIR, "nut-seed-editorial-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/nut-seed-editorial-relationship-report.json");
...
const TIER_COUNT_TARGETS = {
  ...
  B: { min: 75, max: 100 },
  C: { min: 120, max: 170 },
  total: { min: 230, max: 320 },
};

const NUT002_FORBIDDEN_EDITORIAL_PAIRS = [
  ["almond", "almond-flour"],
  ...
  ["pumpkin-seed", "pumpkin-seed-meal"],
  ["pumpkin-seed", "pumpkin-seed-flour"],
  ["sunflower-seed", "sunflower-seed-butter"],
  ["sunflower-seed", "sunflower-seed-flour"],
  ["sunflower-seed", "sunflower-seed-paste"],
  ["hemp-seed", "hemp-seed-butter"],
  ["hemp-seed", "hemp-seed-flour"],
  ["hemp-seed", "hemp-seed-meal"],
...
function validateNut002EditorialRule(entries) {
  const errors = [];
  for (const entry of entries) {
    if (!["similar_to", "substitutes_for"].includes(entry.relationship)) continue;
    for (const [slugA, slugB] of NUT002_FORBIDDEN_EDITORIAL_PAIRS) {
      const matches =
        (entry.source === slugA && entry.target === slugB) ||
        (entry.source === slugB && entry.target === slugA);
      if (matches) {
        errors.push(
          `NUT-002 Editorial Rule: ${entry.relationship} forbidden between ${slugA} and ${slugB}`
        );
      }
    }
  }
...
function validateNut002EditorialEdges(edges, byId) {
  const errors = [];
  for (const edge of edges) {
    ...
    for (const [slugA, slugB] of NUT002_FORBIDDEN_EDITORIAL_PAIRS) {
      ...
      if (matches) {
        errors.push(
          `NUT-002 Editorial Rule: resolved edge ${edge.relationship} ${sourceSlug} <-> ${targetSlug}`
        );
      }
    }
  }
...
function loadInputs() {
  ...
  const indexes = JSON.parse(fs.readFileSync(path.join(RUNTIME_DIR, "nut-seed-indexes.json"), "utf8"));
...
function isCrossDomainTarget(target) {
  return target.startsWith("food.") && !target.startsWith("food.nut-seed.");
}

export function mapNutSeedEditorialRelationships({ slugMap, catalog }) {
  const nut002SeedErrors = validateNut002EditorialRule(EDITORIAL_CURATED);
  if (nut002SeedErrors.length) {
    throw new Error(nut002SeedErrors.join("\n"));
  }
  ...
  for (const entry of EDITORIAL_CURATED) {
    ...
    if (isCrossDomainTarget(entry.target)) {
      ...
      targetId = resolveNutSeedId(slugMap, entry.target);
      if (!targetId) {
  ...
  return {
    meta: {
      ...
      inputs: [
        ...
        "scripts/nut-seed-editorial-seed-10d.js",
      ],
      scope_exclusions: [
        "wine_pairings",
        "preparation_advice",
      ...
      nut001_editorial_rule:
        "NUT-001: culinary group assignment remains authoritative — peanut in Peanuts, almond in Tree Nuts, sesame in Edible Seeds.",
      nut002_editorial_rule:
        "NUT-002: whole and processed nut/seed canonical identities must not be treated as editorially interchangeable in Tier A relationships.",
    },
    edges,
    stats: {
...
function validateEditorial(output, structural, nutSeedIds) {
  const errors = [];
  const seen = new Set();
  ...
  for (const edge of output.edges) {
    ...
    } else if (!nutSeedIds.has(edge.target)) {
      missingEntities += 1;
  ...
  for (const [tier, target] of Object.entries(TIER_COUNT_TARGETS)) {
    if (tier === "total") {
      if (output.edges.length < target.min || output.edges.length > target.max) {
        errors.push(
          `Edge count ${output.edges.length} outside FOOD-10D target ${target.min}-${target.max}`
        );
      }
      continue;
    }
    ...
    if (count < target.min || count > target.max) {
      errors.push(`${tier}: ${count} edges outside FOOD-10D target ${target.min}-${target.max}`);
    }
  }
...
function main() {
  ...
  const output = mapNutSeedEditorialRelationships({ slugMap, catalog });
  const byIdForNut002 = Object.fromEntries(
    Object.entries(slugMap).map(([slug, id]) => [id, { slug, id }])
  );
  const validation = validateEditorial(output, structural, output.nutSeedIds);
  validation.errors.push(...validateNut002EditorialEdges(output.edges, byIdForNut002));
  const rebuilt = mapNutSeedEditorialRelationships({ slugMap, catalog });
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/nut-seed-editorial-relationships.json",
    ...
    nut001_editorial_rule: output.meta.nut001_editorial_rule,
    nut002_editorial_rule: output.meta.nut002_editorial_rule,
  };
