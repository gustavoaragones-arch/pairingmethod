#!/usr/bin/env node
/**
 * FOOD-12D — Editorial relationship mapper for sweet-flavor ontology.
 * Level 3 (editorial) relationships — human-curated, evidence-backed.
 * Does not modify catalog, runtime graph, or structural relationship layer.
 *
 * Run: node scripts/map-sweet-flavor-editorial-relationships-12d.mjs
 */
...
import { EDITORIAL_CURATED, FORWARD_REFERENCE_IDS } from "./sweet-flavor-editorial-seed-12d.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_PATH = path.join(ROOT, "data/runtime/sweet-flavor-runtime.json");
const OUTPUT_PATH = path.join(ROOT, "data/runtime/sweet-flavor-editorial-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/sweet-flavor-editorial-relationship-report.json");
...
const SYMMETRIC_TYPES = new Set(["similar_to", "similar_sweet_flavors"]);

const EDITORIAL_TIERS = {
  A: {
    label: "high_confidence_culinary_equivalence",
    relationships: ["similar_to", "substitutes_for", "similar_sweet_flavors"],
  },
  B: {
...
const SWEET001_COCOA001_FORBIDDEN_EDITORIAL_PAIRS = [
  ["cacao-bean", "cocoa-powder"],
  ["cacao-bean", "natural-cocoa-powder"],
  ["cacao-bean", "dutch-process-cocoa-powder"],
  ["cacao-bean", "cocoa-butter"],
  ["cacao-bean", "chocolate-liquor"],
  ["cacao-bean", "cacao-nibs"],
  ["cacao-bean", "cacao-paste"],
  ["cacao-bean", "cacao-powder"],
  ["cacao-bean", "raw-cacao"],
  ["cacao-bean", "ground-cacao"],
  ["cacao-bean", "toasted-cacao"],
  ["chocolate-liquor", "cocoa-powder"],
  ["chocolate-liquor", "cocoa-butter"],
  ["cacao-paste", "cocoa-powder"],
  ["cacao-nibs", "cocoa-powder"],
  ["cocoa-butter", "cocoa-powder"],
  ["cacao-paste", "chocolate-liquor"],
];
...
function validateSweetEditorialRule(entries) {
  const errors = [];
  for (const entry of entries) {
    if (!["similar_to", "substitutes_for", "similar_sweet_flavors"].includes(entry.relationship)) {
      continue;
    }
    for (const [slugA, slugB] of SWEET001_COCOA001_FORBIDDEN_EDITORIAL_PAIRS) {
      const matches =
        (entry.source === slugA && entry.target === slugB) ||
        (entry.source === slugB && entry.target === slugA);
      if (matches) {
        errors.push(
          `SWEET-001/COCOA-001 Editorial Rule: ${entry.relationship} forbidden between ${slugA} and ${slugB}`
        );
      }
    }
  }
...
function validateSweetEditorialEdges(edges, byId) {
  const errors = [];
  for (const edge of edges) {
    if (!["similar_to", "substitutes_for", "similar_sweet_flavors"].includes(edge.relationship)) {
      continue;
    }
    ...
    for (const [slugA, slugB] of SWEET001_COCOA001_FORBIDDEN_EDITORIAL_PAIRS) {
      ...
      if (matches) {
        errors.push(
          `SWEET-001/COCOA-001 Editorial Rule: resolved edge ${edge.relationship} ${sourceSlug} <-> ${targetSlug}`
        );
      }
    }
  }
...
function isCrossDomainTarget(target) {
  return target.startsWith("food.") && !target.startsWith("food.sweet-flavor.");
}

export function mapSweetFlavorEditorialRelationships({ slugMap, catalogVersion }) {
  const seedErrors = validateSweetEditorialRule(EDITORIAL_CURATED);
  if (seedErrors.length) {
    throw new Error(seedErrors.join("\n"));
  }
  ...
  for (const entry of EDITORIAL_CURATED) {
    ...
    if (isCrossDomainTarget(entry.target)) {
      ...
      targetId = resolveSweetFlavorId(slugMap, entry.target);
      if (!targetId) {
  ...
  const usedTypes = EDITORIAL_RELATIONSHIP_TYPES.filter((type) => relationshipTypeCounts[type] > 0);
  ...
  return {
    meta: {
      ...
      inputs: [
        ...
        "scripts/sweet-flavor-editorial-seed-12d.js",
      ],
      scope_exclusions: [
        "wine_pairings",
      ...
      sweet001_editorial_rule:
        "SWEET-001: crystalline trade names and granulation grades remain aliases — separate canonical entities only when culinary identity changes.",
      cocoa001_editorial_rule:
        "COCOA-001: cacao bean agricultural form and independent cocoa-derived ingredients must not be treated as editorially interchangeable in Tier A.",
    },
    edges,
    stats: {
...
function validateReciprocity(edges, byId) {
  const errors = [];
  const symmetricKeys = new Set();

  for (const edge of edges) {
    if (!SYMMETRIC_TYPES.has(edge.relationship)) continue;
    const sourceSlug = byId[edge.source]?.slug;
    const targetSlug = byId[edge.target]?.slug;
    if (!sourceSlug || !targetSlug) continue;
    const [a, b] = [sourceSlug, targetSlug].sort();
    const pairKey = `${edge.relationship}\t${a}\t${b}`;
    if (symmetricKeys.has(pairKey)) {
      errors.push(`Reciprocity: duplicate symmetric pair ${edge.relationship} ${a} <-> ${b}`);
    }
    symmetricKeys.add(pairKey);
  }

  return errors;
}

function validateNodeCoverage(edges, sweetFlavorIds, byId) {
  const errors = [];
  const connected = new Set();

  for (const edge of edges) {
    if (sweetFlavorIds.has(edge.source)) connected.add(edge.source);
    if (sweetFlavorIds.has(edge.target)) connected.add(edge.target);
  }

  const orphanSlugs = [];
  for (const id of sweetFlavorIds) {
    if (!connected.has(id)) {
      orphanSlugs.push(byId[id]?.slug ?? id);
    }
  }

  if (orphanSlugs.length > 0) {
    errors.push(`Orphan detection: ${orphanSlugs.length} entities without editorial edges`);
  }

  return { errors, orphanCount: orphanSlugs.length, connectedCount: connected.size };
}

function validateEditorial(output, structural, sweetFlavorIds, slugMap) {
  const errors = [];
  const seen = new Set();
  ...
  for (const slug of CROSS_DOMAIN_FORBIDDEN_SLUGS) {
    if (slugMap[slug]) {
      errors.push(`Cross-domain: forbidden sweet-flavor runtime slug ${slug}`);
    }
  }
  ...
  for (const edge of output.edges) {
    ...
    } else if (!sweetFlavorIds.has(edge.target)) {
      missingEntities += 1;
    ...
    if (
      edge.editorial_tier === "A" &&
      !["similar_to", "substitutes_for", "similar_sweet_flavors"].includes(edge.relationship)
    ) {
  ...
  for (const [tier, target] of Object.entries(TIER_COUNT_TARGETS)) {
    if (tier === "total") {
      if (output.edges.length < target.min || output.edges.length > target.max) {
        errors.push(
          `Edge count ${output.edges.length} outside FOOD-12D target ${target.min}-${target.max}`
        );
      }
      continue;
    }
    ...
    if (count < target.min || count > target.max) {
      errors.push(`${tier}: ${count} edges outside FOOD-12D target ${target.min}-${target.max}`);
    }
  }
...
function main() {
  const { slugMap, structural, catalogVersion } = loadInputs();
  const output = mapSweetFlavorEditorialRelationships({ slugMap, catalogVersion });
  const byIdForGovernance = Object.fromEntries(
    Object.entries(slugMap).map(([slug, id]) => [id, { slug, id }])
  );
  const validation = validateEditorial(output, structural, output.sweetFlavorIds, slugMap);
  validation.errors.push(...validateSweetEditorialEdges(output.edges, byIdForGovernance));
  validation.errors.push(...validateReciprocity(output.edges, byIdForGovernance));
  const coverage = validateNodeCoverage(output.edges, output.sweetFlavorIds, byIdForGovernance);
  validation.errors.push(...coverage.errors);

  const rebuilt = mapSweetFlavorEditorialRelationships({ slugMap, catalogVersion });
  ...
  if (overall === "FAIL") {
    const report = {
      ...
      metrics: {
        ...
        "Connected entities": coverage.connectedCount,
        "Orphan entities": coverage.orphanCount,
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/sweet-flavor-editorial-relationships.json",
    ...
    sweet001_editorial_rule: output.meta.sweet001_editorial_rule,
    cocoa001_editorial_rule: output.meta.cocoa001_editorial_rule,
  };
