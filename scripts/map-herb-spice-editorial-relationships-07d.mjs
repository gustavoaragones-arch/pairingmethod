#!/usr/bin/env node
/**
 * FOOD-07D — Editorial relationship mapper for herb & spice ontology.
 ...
 *
 * Run: node scripts/map-herb-spice-editorial-relationships-07d.mjs
 */
...
import { EDITORIAL_CURATED, FORWARD_REFERENCE_IDS } from "./herb-spice-editorial-seed-07d.js";
...
const OUTPUT_PATH = path.join(RUNTIME_DIR, "herb-spice-editorial-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/herb-spice-editorial-relationship-report.json");
...
const BOTANICAL_FORBIDDEN_EDITORIAL_SLUGS = [
  ["cilantro", "coriander-seed"],
  ["dill", "dill-seed"],
];

function validateBotanicalEditorialRule(entries, slugMap) {
  const errors = [];
  for (const entry of entries) {
    if (!["similar_to", "substitutes_for"].includes(entry.relationship)) continue;
    for (const [slugA, slugB] of BOTANICAL_FORBIDDEN_EDITORIAL_SLUGS) {
      const matches =
        (entry.source === slugA && entry.target === slugB) ||
        (entry.source === slugB && entry.target === slugA);
      if (matches) {
        errors.push(
          `Botanical Editorial Rule: ${entry.relationship} forbidden between ${slugA} and ${slugB}`
        );
      }
    }
  }
  for (const [slugA, slugB] of BOTANICAL_FORBIDDEN_EDITORIAL_SLUGS) {
    const idA = slugMap[slugA];
    const idB = slugMap[slugB];
    if (idA && idB && idA === idB) {
      errors.push(`Botanical Editorial Rule: runtime collapsed ${slugA} and ${slugB}`);
    }
  }
  return errors;
}

function validateBotanicalEditorialEdges(edges, byId) {
  const errors = [];
  for (const edge of edges) {
    ...
    for (const [slugA, slugB] of BOTANICAL_FORBIDDEN_EDITORIAL_SLUGS) {
      ...
      if (matches) {
        errors.push(
          `Botanical Editorial Rule: resolved edge ${edge.relationship} ${sourceSlug} <-> ${targetSlug}`
        );
      }
    }
  }
...
function loadInputs() {
  ...
  const indexes = JSON.parse(fs.readFileSync(path.join(RUNTIME_DIR, "herb-spice-indexes.json"), "utf8"));
...
function isCrossDomainTarget(target) {
  return target.startsWith("food.") && !target.startsWith("food.herb.");
}

export function mapHerbSpiceEditorialRelationships({ slugMap, catalog }) {
  const botanicalSeedErrors = validateBotanicalEditorialRule(EDITORIAL_CURATED, slugMap);
  ...
  for (const entry of EDITORIAL_CURATED) {
    ...
    if (isCrossDomainTarget(entry.target)) {
      ...
      targetId = resolveHerbSpiceId(slugMap, entry.target);
      if (!targetId) {
  ...
  return {
    meta: {
      ...
      inputs: [
        ...
        "scripts/herb-spice-editorial-seed-07d.js",
      ],
      scope_exclusions: [
        "wine_pairings",
        "preparation_advice",
      ...
      botanical_editorial_rule:
        "Cilantro and coriander seed, dill and dill seed, must not be treated as interchangeable in Tier A editorial relationships.",
    },
    edges,
    stats: {
...
function validateEditorial(output, structural, herbSpiceIds) {
  const errors = [];
  const seen = new Set();
  ...
  for (const edge of output.edges) {
    ...
    } else if (!herbSpiceIds.has(edge.target)) {
      missingEntities += 1;
...
function main() {
  ...
  const output = mapHerbSpiceEditorialRelationships({ slugMap, catalog });
  const byIdForBotan = Object.fromEntries(Object.entries(slugMap).map(([slug, id]) => [id, { slug, id }]));
  const validation = validateEditorial(output, structural, output.herbSpiceIds);
  validation.errors.push(...validateBotanicalEditorialEdges(output.edges, byIdForBotan));
  const rebuilt = mapHerbSpiceEditorialRelationships({ slugMap, catalog });
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/herb-spice-editorial-relationships.json",
