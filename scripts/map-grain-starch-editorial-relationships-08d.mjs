#!/usr/bin/env node
/**
 * FOOD-08D — Editorial relationship mapper for grain & starch ontology.
 ...
 *
 * Run: node scripts/map-grain-starch-editorial-relationships-08d.mjs
 */
...
import { EDITORIAL_CURATED, FORWARD_REFERENCE_IDS } from "./grain-starch-editorial-seed-08d.js";
...
const OUTPUT_PATH = path.join(RUNTIME_DIR, "grain-starch-editorial-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/grain-starch-editorial-relationship-report.json");
...
const TIER_COUNT_TARGETS = {
  ...
  B: { min: 45, max: 70 },
  C: { min: 80, max: 120 },
  total: { min: 150, max: 230 },
};

const PROC_FORBIDDEN_EDITORIAL_PAIRS = [
  ["wheat", "wheat-flour"],
...
function validateProcessingEditorialRule(entries) {
  const errors = [];
  for (const entry of entries) {
    if (!["similar_to", "substitutes_for"].includes(entry.relationship)) continue;
    for (const [slugA, slugB] of PROC_FORBIDDEN_EDITORIAL_PAIRS) {
      const matches =
        (entry.source === slugA && entry.target === slugB) ||
        (entry.source === slugB && entry.target === slugA);
      if (matches) {
        errors.push(
          `PROC-001 Editorial Rule: ${entry.relationship} forbidden between ${slugA} and ${slugB}`
        );
      }
    }
  }
...
function validateProcessingEditorialEdges(edges, byId) {
  const errors = [];
  for (const edge of edges) {
    ...
    for (const [slugA, slugB] of PROC_FORBIDDEN_EDITORIAL_PAIRS) {
      ...
      if (matches) {
        errors.push(
          `PROC-001 Editorial Rule: resolved edge ${edge.relationship} ${sourceSlug} <-> ${targetSlug}`
        );
      }
    }
  }
...
function loadInputs() {
  ...
  const indexes = JSON.parse(fs.readFileSync(path.join(RUNTIME_DIR, "grain-starch-indexes.json"), "utf8"));
...
function isCrossDomainTarget(target) {
  return target.startsWith("food.") && !target.startsWith("food.grain.");
}

export function mapGrainStarchEditorialRelationships({ slugMap, catalog }) {
  const procSeedErrors = validateProcessingEditorialRule(EDITORIAL_CURATED);
  ...
  for (const entry of EDITORIAL_CURATED) {
    ...
    if (isCrossDomainTarget(entry.target)) {
      ...
      targetId = resolveGrainStarchId(slugMap, entry.target);
      if (!targetId) {
  ...
  return {
    meta: {
      ...
      inputs: [
        ...
        "scripts/grain-starch-editorial-seed-08d.js",
      ],
      scope_exclusions: [
        "wine_pairings",
        "preparation_advice",
      ...
      processing_editorial_rule:
        "PROC-001: processing-derived canonical identities must not be treated as editorially interchangeable in Tier A relationships.",
    },
    edges,
    stats: {
...
function validateEditorial(output, structural, grainStarchIds) {
  const errors = [];
  const seen = new Set();
  ...
  for (const edge of output.edges) {
    ...
    } else if (!grainStarchIds.has(edge.target)) {
      missingEntities += 1;
  ...
  for (const [tier, target] of Object.entries(TIER_COUNT_TARGETS)) {
    if (tier === "total") {
      if (output.edges.length < target.min || output.edges.length > target.max) {
        errors.push(
          `Edge count ${output.edges.length} outside FOOD-08D target ${target.min}-${target.max}`
        );
      }
      continue;
    }
    ...
    if (count < target.min || count > target.max) {
      errors.push(`${tier}: ${count} edges outside FOOD-08D target ${target.min}-${target.max}`);
    }
  }
...
function main() {
  ...
  const output = mapGrainStarchEditorialRelationships({ slugMap, catalog });
  const byIdForProc = Object.fromEntries(
    Object.entries(slugMap).map(([slug, id]) => [id, { slug, id }])
  );
  const validation = validateEditorial(output, structural, output.grainStarchIds);
  validation.errors.push(...validateProcessingEditorialEdges(output.edges, byIdForProc));
  const rebuilt = mapGrainStarchEditorialRelationships({ slugMap, catalog });
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/grain-starch-editorial-relationships.json",
    ...
    processing_editorial_rule: output.meta.processing_editorial_rule,
  };
