#!/usr/bin/env node
/**
 * FOOD-11C — Compile legume catalog into consolidated runtime graph.
 * Read-only compiler: never modifies data/legume-catalog.json.
 *
 * Run: node scripts/build-legume-runtime-11c.js
 */
...
export const RUNTIME_STABILITY_LEVELS = {
  ...
  level_3_editorial: ["similar_to", "substitutes_for", "commonly_served_with", "similar_legumes"],
};
...
function buildVocabularyIndex(legumes) {
...
function buildAliasIndex(legumes) {
  const byAlias = {};
  for (const entity of legumes) {
...
function buildNamespaceLookup(categories, groups, legumes) {
  const byNamespace = {
    "food.legume": categories.map((c) => c.id).sort(),
  };
  for (const group of groups) {
    byNamespace[group.id] = legumes
...
function mapRelationships({ categories, groups, byId }) {
  ...
  for (const group of groups) {
    ...
    for (const entityId of group.legume_ids) {
...
export function compileLegumeRuntime(catalog) {
  const legumes = [...(catalog.legumes ?? [])].sort((a, b) => a.id.localeCompare(b.id));
  ...
  for (const entity of legumes) {
    byId[entity.id] = legumeRef(entity);
  ...
  for (const group of groups) {
    ...
    const childIds = group.child_slugs
      ...
      .filter(Boolean)
      .sort();
    groupToLegumes[group.slug] = childIds;
  }

  for (const entity of legumes) {
    legumeToGroup[entity.slug] = entity.parent_group;
  }
  ...
  const groupsOut = groups.map((group) => ({
    ...
    legume_slugs: [...group.child_slugs].sort(),
    legume_ids: group.child_slugs.map((s) => bySlug[s]).sort(),
  }));

  const vocabularyIndex = buildVocabularyIndex(legumes);
  const byNamespace = buildNamespaceLookup(categories, groups, legumes);
  const byAlias = buildAliasIndex(legumes);
  ...
  return {
    meta: {
      domain: "legume",
      source: "data/legume-catalog.json",
      ...
      runtime_stability_levels: {
        ...
        level_2_intrinsic_similarity: "shares_usage_intensity, shares_scientific_name",
        level_3_editorial: "reserved FOOD-11D — not generated in runtime compile",
      },
    },
    categories: categoriesOut,
    groups: groupsOut,
    hierarchy: {
      ...
      group_to_legumes: groupToLegumes,
      legume_to_group: legumeToGroup,
    },
    indexes: {
      by_id: byId,
      by_slug: bySlug,
...
function validateForBuild(catalog) {
  ...
  for (const entity of legumes) {
    ...
    if (entity.parent_category !== "legume") {
      errors.push(`${entity.slug}: parent_category must be legume`);
    }
    ...
    for (const field of RESERVED_PROFILE_FIELDS) {
      if (!Array.isArray(entity[field]) || entity[field].length > 0) {
        errors.push(`${entity.slug}: ${field} must be empty array in FOOD-11C`);
      }
    }
  }
...
function main() {
  ...
  const runtime = compileLegumeRuntime(catalog);
  ...
  const report = {
    ...
    metrics: {
      ...
      "Legume entities indexed": runtime.meta.entity_count,
