#!/usr/bin/env node
/**
 * FOOD-11D — Editorial relationship mapper for legume ontology.
 ...
 *
 * Run: node scripts/map-legume-editorial-relationships-11d.mjs
 */
...
import { EDITORIAL_CURATED, FORWARD_REFERENCE_IDS } from "./legume-editorial-seed-11d.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_PATH = path.join(ROOT, "data/runtime/legume-runtime.json");
const OUTPUT_PATH = path.join(ROOT, "data/runtime/legume-editorial-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/legume-editorial-relationship-report.json");
const EDGE_VERSION = "1.0";

const EDITORIAL_RELATIONSHIP_TYPES = ["similar_to", "commonly_served_with"];

const SYMMETRIC_TYPES = new Set(["similar_to"]);

const EDITORIAL_TIERS = {
  A: {
    label: "closely_related_legumes_and_processed_forms",
...
const TIER_COUNT_TARGETS = {
  ...
  B: { min: 70, max: 95 },
  C: { min: 110, max: 160 },
  total: { min: 240, max: 300 },
};

const LEGUME002_FORBIDDEN_EDITORIAL_PAIRS = [
  ...
  ["soybean", "soy-flour"],
  ["soybean", "natto"],
  ["soybean", "textured-vegetable-protein"],
  ["soybean", "soy-curls"],
  ["soybean", "fermented-black-bean"],
...
function validateLegume002EditorialRule(entries) {
  const errors = [];
  for (const entry of entries) {
    if (entry.relationship !== "similar_to") continue;
    for (const [slugA, slugB] of LEGUME002_FORBIDDEN_EDITORIAL_PAIRS) {
      const matches =
        (entry.source === slugA && entry.target === slugB) ||
        (entry.source === slugB && entry.target === slugA);
      if (matches) {
        errors.push(
          `LEGUME-002 Editorial Rule: ${entry.relationship} forbidden between ${slugA} and ${slugB}`
        );
      }
    }
  }
...
function validateLegume002EditorialEdges(edges, byId) {
  const errors = [];
  for (const edge of edges) {
    if (edge.relationship !== "similar_to") continue;
    ...
    for (const [slugA, slugB] of LEGUME002_FORBIDDEN_EDITORIAL_PAIRS) {
      ...
      if (matches) {
        errors.push(
          `LEGUME-002 Editorial Rule: resolved edge ${edge.relationship} ${sourceSlug} <-> ${targetSlug}`
        );
      }
    }
  }
...
function isCrossDomainTarget(target) {
  return target.startsWith("food.") && !target.startsWith("food.legume.");
}

export function mapLegumeEditorialRelationships({ slugMap, catalogVersion }) {
  const legume002SeedErrors = validateLegume002EditorialRule(EDITORIAL_CURATED);
  if (legume002SeedErrors.length) {
    throw new Error(legume002SeedErrors.join("\n"));
  }
  ...
  for (const entry of EDITORIAL_CURATED) {
    ...
    if (isCrossDomainTarget(entry.target)) {
      ...
      targetId = resolveLegumeId(slugMap, entry.target);
      if (!targetId) {
  ...
  return {
    meta: {
      ...
      inputs: [
        ...
        "scripts/legume-editorial-seed-11d.js",
      ],
      scope_exclusions: [
        "wine_pairings",
        "substitutions",
      ...
      legume001_editorial_rule:
        "LEGUME-001: culinary group assignment remains authoritative — soybean in Other Legumes, chickpea in Chickpeas, peanut excluded from Legume domain.",
      legume002_editorial_rule:
        "LEGUME-002: whole and processed legume canonical identities must not be treated as editorially interchangeable in Tier A relationships.",
    },
    edges,
    stats: {
...
function validateEditorial(output, structural, legumeIds, slugMap) {
  const errors = [];
  const seen = new Set();
  ...
  let conflicts = 0;
  let pendingReview = 0;
  let substitutionViolations = 0;

  for (const slug of CROSS_DOMAIN_FORBIDDEN_SLUGS) {
    if (slugMap[slug]) {
      errors.push(`Cross-domain: forbidden legume runtime slug ${slug}`);
    }
  }
  ...
  for (const edge of output.edges) {
    ...
    } else if (!legumeIds.has(edge.target)) {
      missingEntities += 1;
    ...
    if (edge.relationship === "substitutes_for") {
      substitutionViolations += 1;
      errors.push(`Substitutions forbidden in FOOD-11D: ${key}`);
    }

    if (edge.editorial_tier === "A" && edge.relationship !== "similar_to") {
  ...
  for (const [tier, target] of Object.entries(TIER_COUNT_TARGETS)) {
    if (tier === "total") {
      if (output.edges.length < target.min || output.edges.length > target.max) {
        errors.push(
          `Edge count ${output.edges.length} outside FOOD-11D target ${target.min}-${target.max}`
        );
      }
      continue;
    }
    ...
    if (count < target.min || count > target.max) {
      errors.push(`${tier}: ${count} edges outside FOOD-11D target ${target.min}-${target.max}`);
    }
  }
...
function main() {
  ...
  const output = mapLegumeEditorialRelationships({ slugMap, catalogVersion });
  const byIdForLegume002 = Object.fromEntries(
    Object.entries(slugMap).map(([slug, id]) => [id, { slug, id }])
  );
  const validation = validateEditorial(output, structural, output.legumeIds, slugMap);
  validation.errors.push(...validateLegume002EditorialEdges(output.edges, byIdForLegume002));
  const rebuilt = mapLegumeEditorialRelationships({ slugMap, catalogVersion });
  ...
  if (overall === "FAIL") {
    const report = {
      ...
      metrics: {
        ...
        "Substitution violations": validation.substitutionViolations,
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/legume-editorial-relationships.json",
    ...
    legume001_editorial_rule: output.meta.legume001_editorial_rule,
    legume002_editorial_rule: output.meta.legume002_editorial_rule,
  };
