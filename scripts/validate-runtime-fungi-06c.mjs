#!/usr/bin/env node
/**
 * FOOD-06C.2 — Runtime validator for fungi bootstrap artifacts.
 * Structural verification only: no catalog writes, no repairs, no relationships.
 *
 * Run: node scripts/validate-runtime-fungi-06c.mjs
 * Output: reports/fungi-runtime-validator-report.json
 */
...
import {
  ...
  serializeRuntime,
  compileFungiRuntime,
} from "./bootstrap-fungi-runtime.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const CATALOG_PATH = path.join(ROOT, "data/fungi-catalog.json");
const REPORT_PATH = path.join(ROOT, "reports/fungi-runtime-validator-report.json");

const ARTIFACT_BUILDERS = {
  "fungi-bootstrap.json": (built) => built.bootstrapDoc,
  "fungi-categories.json": (built) => built.categoriesOut,
  "fungi-groups.json": (built) => built.groupsOut,
  "fungi-indexes.json": (built) => built.indexesDoc,
};
...
function validateHierarchy({ bootstrap, categories, groups, bySlug, byId, errors, checks }) {
  ...
  const { category_to_groups, group_to_category, group_to_fungi, fungus_to_group } = bootstrap.hierarchy;
  ...
  for (const group of groups) {
    for (const fungusSlug of group.fungus_slugs) {
      if (!bySlug[fungusSlug]) {
        errors.push(`Hierarchy: group ${group.slug} references missing fungus slug ${fungusSlug}`);
      }
    }
    const expectedIds = group.fungus_slugs.map((s) => bySlug[s]).filter(Boolean).sort();
    if (JSON.stringify(group.fungus_ids) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} fungus_ids mismatch fungus_slugs`);
    }
    const indexedFungi = group_to_fungi[group.slug] ?? [];
    if (JSON.stringify(indexedFungi) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} bootstrap fungus list mismatch`);
    }
  ...
  for (const [fungusSlug, parentGroup] of Object.entries(fungus_to_group)) {
    if (!bySlug[fungusSlug]) {
      errors.push(`Hierarchy: fungus_to_group references unknown slug ${fungusSlug}`);
      continue;
    }
    if (!groupSlugs.has(parentGroup)) {
      errors.push(`Hierarchy: fungus ${fungusSlug} parent group ${parentGroup} missing`);
      continue;
    }
    const ref = byId[bySlug[fungusSlug]];
    if (ref.parent_group !== parentGroup) {
      errors.push(`Hierarchy: fungus ${fungusSlug} parent_group ${ref.parent_group} != index ${parentGroup}`);
    }
    const group = groups.find((g) => g.slug === parentGroup);
    if (group && !group.fungus_slugs.includes(fungusSlug)) {
      errors.push(`Hierarchy: reciprocal failure — ${fungusSlug} not in group ${parentGroup} fungus_slugs`);
    }
  }
  if (!errors.some((e) => e.startsWith("Hierarchy: fungus") || e.startsWith("Hierarchy: reciprocal"))) {
    passed += 1;
  }
...
function validateByGroup({ byGroup, byId, groups, errors, checks }) {
  ...
  for (const group of groups) {
    const expected = group.fungus_ids.sort();
    const indexed = (byGroup[group.slug] ?? []).slice().sort();
    if (JSON.stringify(indexed) !== JSON.stringify(expected)) {
      errors.push(`By group: ${group.slug} index mismatch with group fungus_ids`);
    }
  }
...
function validateCrossIndex({ bootstrap, groups, bySlug, byId, byUsageIntensity, byGroup, vocabularyIndex, errors, checks }) {
  ...
  for (const group of groups) {
    for (const id of group.fungus_ids) {
      const ref = byId[id];
      if (!ref) {
        errors.push(`Cross-index: group ${group.slug} fungus_id ${id} missing from by_id`);
        continue;
      }
      if (bySlug[ref.slug] !== id) errors.push(`Cross-index: group fungus ${id} by_slug mismatch`);
      if (bootstrap.hierarchy.fungus_to_group[ref.slug] !== group.slug) {
        errors.push(`Cross-index: group ${group.slug} fungus ${ref.slug} hierarchy mismatch`);
      }
    }
  }
...
function validateNamespace({ byNamespace, categories, groups, byId, errors, checks }) {
  ...
  const nsCategory = byNamespace["food.fungi"] ?? [];

  if (JSON.stringify(nsCategory) !== JSON.stringify(categoryIds.sort())) {
    errors.push("Namespace: food.fungi namespace mismatch with categories");
  } else passed += 1;
  ...
  for (const group of groups) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      errors.push(`Namespace: ${group.id} namespace mismatch with group fungus_ids`);
    }
  }
...
function validateDeterminism(loaded, errors, checks) {
  ...
  const rebuilt = compileFungiRuntime(catalog);
...
function main() {
  ...
  const bootstrap = loaded["fungi-bootstrap.json"];
  const categories = loaded["fungi-categories.json"];
  const groups = loaded["fungi-groups.json"];
  const indexes = loaded["fungi-indexes.json"];
