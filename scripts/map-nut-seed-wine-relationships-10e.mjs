#!/usr/bin/env node
/**
 * FOOD-10E — Wine pairing layer for nut-seed ontology.
 ...
 *
 * Run: node scripts/map-nut-seed-wine-relationships-10e.mjs
 */

import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { serializeRuntime } from "./bootstrap-nut-seed-runtime.js";
...
import { PAIRING_CURATED } from "./nut-seed-wine-seed-10e.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const CATALOG_PATH = path.join(ROOT, "data/nut-seed-catalog.json");
const STRUCTURAL_PATH = path.join(RUNTIME_DIR, "nut-seed-relationships.json");
const EDITORIAL_PATH = path.join(RUNTIME_DIR, "nut-seed-editorial-relationships.json");
const OUTPUT_PATH = path.join(RUNTIME_DIR, "nut-seed-wine-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/nut-seed-wine-relationship-report.json");
...
const TYPE_COUNT_TARGETS = {
  pairs_with_style: { min: 85, max: 95 },
  also_pairs_with_style: { min: 45, max: 60 },
  pairs_with_descriptor: { min: 30, max: 40 },
  pairs_with_technique: { min: 20, max: 30 },
  total: { min: 180, max: 225 },
};

const NUT002_PAIRING_FAMILIES = [
  ["almond", "almond-flour", "almond-butter", "almond-meal"],
  ["peanut", "peanut-butter"],
  ["sesame", "tahini", "sesame-paste", "sesame-flour"],
  ["hazelnut", "hazelnut-flour", "hazelnut-butter", "hazelnut-meal"],
  ["walnut", "walnut-butter", "walnut-meal"],
  ["pecan", "pecan-butter", "pecan-meal"],
  ["cashew", "cashew-butter", "cashew-flour"],
  ["pistachio", "pistachio-paste"],
  ["pine-nut", "pine-nut-paste"],
  ["flaxseed", "flaxseed-meal", "flaxseed-butter"],
  ["chia-seed", "chia-seed-meal"],
  ["pumpkin-seed", "pumpkin-seed-butter", "pumpkin-seed-meal", "pumpkin-seed-flour"],
  ["sunflower-seed", "sunflower-seed-butter", "sunflower-seed-flour", "sunflower-seed-paste"],
  ["hemp-seed", "hemp-seed-butter", "hemp-seed-flour", "hemp-seed-meal"],
  ["macadamia", "macadamia-butter"],
  ["chestnut", "chestnut-flour"],
  ["brazil-nut", "brazil-nut-meal"],
];
...
function loadNutSeedInputs() {
  const catalog = JSON.parse(fs.readFileSync(CATALOG_PATH, "utf8"));
  const indexes = JSON.parse(
    fs.readFileSync(path.join(RUNTIME_DIR, "nut-seed-indexes.json"), "utf8")
  );
...
function resolveNutSeedId(slugMap, slug) {
...
function validateNut002PairingRule(entries) {
  const errors = [];
  const bySlug = Object.fromEntries(
    NUT002_PAIRING_FAMILIES.flat().map((slug) => [slug, entries.filter((e) => e.source === slug)])
  );

  for (const family of NUT002_PAIRING_FAMILIES) {
    ...
    for (let i = 0; i < signatures.length; i += 1) {
      for (let j = i + 1; j < signatures.length; j += 1) {
        ...
        if (a.signature && a.signature === b.signature) {
          errors.push(
            `NUT-002: identical pairing profile forbidden between ${a.slug} and ${b.slug}`
          );
        }
      }
    }
...
function validateNutPair001Rule(entries) {
  const errors = [];
  const roleChecks = [
    { slug: "almond-flour", terms: ["baking", "pastry", "macaron", "gluten-free"] },
    { slug: "peanut-butter", terms: ["spread", "creamy", "sauce", "rich"] },
    { slug: "tahini", terms: ["sesame", "savory", "hummus", "paste"] },
    { slug: "pumpkin-seed", terms: ["roasted", "garnish", "mole"] },
    { slug: "chia-seed", terms: ["pudding", "bowl", "seed"] },
    { slug: "peanut", terms: ["satay", "sauce", "savory", "peanut"] },
    { slug: "almond", terms: ["marzipan", "crust", "pastry", "baking"] },
    { slug: "sesame", terms: ["seed", "garnish", "sesame", "baking"] },
  ];

  for (const entry of entries) {
    if (!entry.evidence.toLowerCase().includes("nut-pair-001")) {
      errors.push(
        `NUT-PAIR-001: ${entry.source} pairing evidence must cite NUT-PAIR-001 culinary function rule`
      );
    }
  }
  ...
  for (const { slug, terms } of roleChecks) {
    ...
    for (const entry of styleEntries) {
      ...
      if (!terms.some((term) => evidence.includes(term))) {
        errors.push(
          `NUT-PAIR-001: ${slug} pairing must cite ${terms.join("/")} culinary function`
        );
      }
    }
  }
...
function validateNut001Grouping(slugMap, catalog) {
  const errors = [];
  const assignments = [
    { slug: "peanut", group: "peanuts" },
    { slug: "almond", group: "tree-nuts" },
    { slug: "sesame", group: "edible-seeds" },
  ];
  for (const { slug, group } of assignments) {
    const entity = catalog.nut_seeds.find((e) => e.slug === slug);
    if (!entity || entity.parent_group !== group) {
      errors.push(`NUT-001: ${slug} must remain in ${group}`);
    }
    if (!slugMap[slug]) {
      errors.push(`NUT-001: runtime missing canonical entity ${slug}`);
    }
  }
  return errors;
}

export function mapNutSeedWineRelationships({ slugMap, catalog, wine }) {
  const nut002SeedErrors = validateNut002PairingRule(PAIRING_CURATED);
  const nutPair001SeedErrors = validateNutPair001Rule(PAIRING_CURATED);
  const nut001Errors = validateNut001Grouping(slugMap, catalog);
  if (nut002SeedErrors.length || nutPair001SeedErrors.length || nut001Errors.length) {
    throw new Error([...nut002SeedErrors, ...nutPair001SeedErrors, ...nut001Errors].join("\n"));
  }

  const nutSeedIds = new Set(Object.values(slugMap));
  const edges = [];
  const seen = new Set();

  for (const entry of PAIRING_CURATED) {
    const sourceId = resolveNutSeedId(slugMap, entry.source);
    if (!sourceId) {
      throw new Error(`Unknown nut or seed slug: ${entry.source}`);
    }
  ...
  const nutSeedsLinked = new Set(edges.map((e) => e.source));
  ...
  return {
    meta: {
      ...
      layer: "wine_pairing",
      pairing_philosophy:
        "Curated editorial pairing knowledge for nuts and seeds — NUT-PAIR-001 pairs by culinary function, not botanical classification alone.",
      culinary_role_pairing_rule:
        "NUT-PAIR-001: wine pairing follows the ingredient's culinary role — almond flour for baking, peanut butter for rich spreads, tahini for savory sesame paste, pumpkin seed for roasted garnish, chia seed for lightly prepared applications.",
      nut002_ownership_rule:
        "NUT-002: processed canonical entities retain distinct pairing profiles and must not inherit wine relationships from whole nut or seed forms.",
      nut001_classification_rule:
        "NUT-001: culinary group assignment remains authoritative in pairing semantics — peanut in Peanuts, almond in Tree Nuts, sesame in Edible Seeds.",
      ...
      inputs: [
        ...
        "scripts/nut-seed-wine-seed-10e.js",
      ],
    },
    edges,
    stats: {
      nut_seeds_linked: nutSeedsLinked.size,
...
function validatePairing(output, wine, nutSeedIds, structural, editorial, catalog) {
  const errors = [];
  const seen = new Set();
  ...
  let duplicates = 0;
  let missingNutSeed = 0;
  let missingWine = 0;
  let pendingReview = 0;

  if (output.stats.nut_seeds_linked !== catalog.nut_seeds.length) {
    errors.push(
      `Catalog coverage: ${output.stats.nut_seeds_linked}/${catalog.nut_seeds.length} entities linked`
    );
  }
  ...
  for (const edge of output.edges) {
    ...
    if (!nutSeedIds.has(edge.source)) {
      missingNutSeed += 1;
      errors.push(`Missing nut or seed entity: ${edge.source}`);
    }
  ...
  for (const [type, target] of Object.entries(TYPE_COUNT_TARGETS)) {
    if (type === "total") {
      if (output.edges.length < target.min || output.edges.length > target.max) {
        errors.push(
          `Edge count ${output.edges.length} outside FOOD-10E target ${target.min}-${target.max}`
        );
      }
      continue;
    }
    ...
    if (count < target.min || count > target.max) {
      errors.push(`${type}: ${count} edges outside FOOD-10E target ${target.min}-${target.max}`);
    }
  }

  return { errors, duplicates, missingNutSeed, missingWine, pendingReview };
}

function main() {
  const { catalog, slugMap, editorial, structural } = loadNutSeedInputs();
  const wine = loadWineOntology();
  const output = mapNutSeedWineRelationships({ slugMap, catalog, wine });
  ...
  const rebuilt = mapNutSeedWineRelationships({ slugMap, catalog, wine });
  ...
  if (overall === "FAIL") {
    const report = {
      ...
      metrics: {
        "Nut & seed entities linked": output.stats.nut_seeds_linked,
        ...
        "Missing nut or seed entities": validation.missingNutSeed,
  ...
  const report = {
    ...
    overall_result: "PASS",
    validation_errors: [],
    output: "data/runtime/nut-seed-wine-relationships.json",
    metrics: {
      "Nut & seed entities linked": output.stats.nut_seeds_linked,
      "Wine styles linked": output.stats.wine_styles_linked,
      "Descriptor links": output.stats.descriptor_links,
      ...
      "Duplicate edges": 0,
      "Missing nut or seed entities": 0,
    ...
    nut002_ownership_rule: output.meta.nut002_ownership_rule,
    nut001_classification_rule: output.meta.nut001_classification_rule,
  };
