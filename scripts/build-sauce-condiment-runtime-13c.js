#!/usr/bin/env node
/**
 * FOOD-13C — Compile sauce-condiment catalog into consolidated runtime graph.
 * Read-only compiler: never modifies data/sauce-condiment-catalog.json.
 *
 * Run: node scripts/build-sauce-condiment-runtime-13c.js
 */
...
function sauceCondimentRef(entity) {
  return {
    id: entity.id,
    slug: entity.slug,
...
function buildVocabularyIndex(sauceCondiments) {
...
function buildAliasIndex(sauceCondiments) {
  const byAlias = {};
  for (const entity of sauceCondiments) {
...
function buildNamespaceLookup(categories, groups, sauceCondiments) {
  const byNamespace = {
    "food.sauce-condiment": categories.map((c) => c.id).sort(),
  };
  for (const group of groups) {
    byNamespace[group.id] = sauceCondiments
...
function mapRelationships({ categories, groups, byId }) {
  ...
  for (const group of groups) {
    ...
    for (const entityId of group.sauce_condiment_ids) {
...
export function compileSauceCondimentRuntime(catalog) {
  const sauceCondiments = [...(catalog.sauce_condiments ?? [])].sort((a, b) => a.id.localeCompare(b.id));
  ...
  for (const entity of sauceCondiments) {
    byId[entity.id] = sauceCondimentRef(entity);
  ...
  const groupToSauceCondiments = {};
  const sauceCondimentToGroup = {};
  ...
  for (const group of groups) {
    ...
    const childIds = group.child_slugs
      ...
      .filter(Boolean)
      .sort();
    groupToSauceCondiments[group.slug] = childIds;
  }

  for (const entity of sauceCondiments) {
    sauceCondimentToGroup[entity.slug] = entity.parent_group;
  }
  ...
  const groupsOut = groups.map((group) => ({
    ...
    sauce_condiment_slugs: [...group.child_slugs].sort(),
    sauce_condiment_ids: group.child_slugs.map((s) => bySlug[s]).sort(),
  }));

  const vocabularyIndex = buildVocabularyIndex(sauceCondiments);
  const byNamespace = buildNamespaceLookup(categories, groups, sauceCondiments);
  const byAlias = buildAliasIndex(sauceCondiments);
  ...
  return {
    meta: {
      domain: "sauce-condiment",
      source: "data/sauce-condiment-catalog.json",
      ...
      runtime_stability_levels: {
        ...
        level_2_intrinsic_similarity: "shares_usage_intensity, shares_scientific_name",
        level_3_editorial: "reserved FOOD-13D — not generated in runtime compile",
      },
    },
    categories: categoriesOut,
    groups: groupsOut,
    hierarchy: {
      ...
      group_to_sauce_condiments: groupToSauceCondiments,
      sauce_condiment_to_group: sauceCondimentToGroup,
    },
    indexes: {
      by_id: byId,
      by_slug: bySlug,
...
function validateForBuild(catalog) {
  ...
  const sauceCondiments = catalog.sauce_condiments ?? [];
  ...
  for (const entity of sauceCondiments) {
    ...
    if (entity.parent_category !== "sauce-condiment") {
      errors.push(`${entity.slug}: parent_category must be sauce-condiment`);
    }
    ...
    if (!entity.scientific_name) {
      errors.push(`${entity.slug}: missing scientific_name`);
    } else if (
      entity.scientific_name !== "Multiple sources" &&
      !entity.scientific_name.includes(" ")
    ) {
      errors.push(`${entity.slug}: invalid scientific_name "${entity.scientific_name}"`);
    }

    for (const field of RESERVED_PROFILE_FIELDS) {
      if (!Array.isArray(entity[field]) || entity[field].length > 0) {
        errors.push(`${entity.slug}: ${field} must be empty array in FOOD-13C`);
      }
    }
  }
...
function main() {
  ...
  const runtime = compileSauceCondimentRuntime(catalog);
  ...
  const report = {
    ...
    metrics: {
      ...
      "Sauce & condiment entities indexed": runtime.meta.entity_count,
