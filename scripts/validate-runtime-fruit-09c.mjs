#!/usr/bin/env node
/**
 * FOOD-09C.2 — Runtime validator for fruit bootstrap artifacts.
 * Structural verification only: no catalog writes, no repairs, no relationships.
 *
 * Run: node scripts/validate-runtime-fruit-09c.mjs
 * Output: reports/fruit-runtime-validator-report.json
 */
...
import {
  ...
  serializeRuntime,
  compileFruitRuntime,
} from "./bootstrap-fruit-runtime.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const CATALOG_PATH = path.join(ROOT, "data/fruit-catalog.json");
const REPORT_PATH = path.join(ROOT, "reports/fruit-runtime-validator-report.json");

const ARTIFACT_BUILDERS = {
  "fruit-bootstrap.json": (built) => built.bootstrapDoc,
  "fruit-categories.json": (built) => built.categoriesOut,
  "fruit-groups.json": (built) => built.groupsOut,
  "fruit-indexes.json": (built) => built.indexesDoc,
};

const FRUIT001_SEPARATED = [
  { slug: "grape", group: "berries", partner: "raisin", partnerGroup: "processed-fruits" },
  { slug: "plum", group: "stone-fruits", partner: "prune", partnerGroup: "processed-fruits" },
  {
    slug: "coconut",
    group: "tropical-fruits",
    partner: "desiccated-coconut",
    partnerGroup: "processed-fruits",
  },
  {
    slug: "coconut",
    group: "tropical-fruits",
    partner: "coconut-milk",
    partnerGroup: "processed-fruits",
  },
];
...
function validateHierarchy({ bootstrap, categories, groups, bySlug, byId, errors, checks }) {
  ...
  const { category_to_groups, group_to_category, group_to_fruits, fruit_to_group } =
    bootstrap.hierarchy;
  ...
  for (const group of groups) {
    for (const fruitSlug of group.fruit_slugs) {
      if (!bySlug[fruitSlug]) {
        errors.push(
          `Hierarchy: group ${group.slug} references missing fruit slug ${fruitSlug}`
        );
      }
    }
    const expectedIds = group.fruit_slugs.map((s) => bySlug[s]).filter(Boolean).sort();
    if (JSON.stringify(group.fruit_ids) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} fruit_ids mismatch fruit_slugs`);
    }
    const indexedGrainStarches = group_to_fruits[group.slug] ?? [];
    if (JSON.stringify(indexedGrainStarches) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} bootstrap fruit list mismatch`);
    }
  ...
  for (const [fruitSlug, parentGroup] of Object.entries(fruit_to_group)) {
    if (!bySlug[fruitSlug]) {
      errors.push(`Hierarchy: fruit_to_group references unknown slug ${fruitSlug}`);
      continue;
    }
    if (!groupSlugs.has(parentGroup)) {
      errors.push(`Hierarchy: fruit ${fruitSlug} parent group ${parentGroup} missing`);
      continue;
    }
    const ref = byId[bySlug[fruitSlug]];
    if (ref.parent_group !== parentGroup) {
      errors.push(
        `Hierarchy: fruit ${fruitSlug} parent_group ${ref.parent_group} != index ${parentGroup}`
      );
    }
    const group = groups.find((g) => g.slug === parentGroup);
    if (group && !group.fruit_slugs.includes(fruitSlug)) {
      errors.push(
        `Hierarchy: reciprocal failure — ${fruitSlug} not in group ${parentGroup} fruit_slugs`
      );
    }
  }
  if (
    !errors.some(
      (e) => e.startsWith("Hierarchy: fruit") || e.startsWith("Hierarchy: reciprocal")
    )
  ) {
    passed += 1;
  }
...
function validateByGroup({ byGroup, byId, groups, errors, checks }) {
  ...
  for (const group of groups) {
    const expected = group.fruit_ids.sort();
    const indexed = (byGroup[group.slug] ?? []).slice().sort();
    if (JSON.stringify(indexed) !== JSON.stringify(expected)) {
      errors.push(`By group: ${group.slug} index mismatch with group fruit_ids`);
    }
  }
...
function validateCrossIndex({
  ...
  for (const group of groups) {
    for (const id of group.fruit_ids) {
      const ref = byId[id];
      if (!ref) {
        errors.push(`Cross-index: group ${group.slug} fruit_id ${id} missing from by_id`);
        continue;
      }
      if (bySlug[ref.slug] !== id) errors.push(`Cross-index: group fruit ${id} by_slug mismatch`);
      if (bootstrap.hierarchy.fruit_to_group[ref.slug] !== group.slug) {
        errors.push(`Cross-index: group ${group.slug} fruit ${ref.slug} hierarchy mismatch`);
      }
    }
  }
...
function validateNamespace({ byNamespace, categories, groups, byId, errors, checks }) {
  ...
  const nsCategory = byNamespace["food.fruit"] ?? [];

  if (JSON.stringify(nsCategory) !== JSON.stringify(categoryIds.sort())) {
    errors.push("Namespace: food.fruit namespace mismatch with categories");
  } else passed += 1;
  ...
  for (const group of groups) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      errors.push(`Namespace: ${group.id} namespace mismatch with group fruit_ids`);
    }
  }
...
function validateFruit001Ownership({ byId, bySlug, errors, checks }) {
  let passed = 0;
  ...
  for (const rule of FRUIT001_SEPARATED) {
    const idA = bySlug[rule.slug];
    const idB = bySlug[rule.partner];
    if (!idA) errors.push(`FRUIT-001: missing canonical entity ${rule.slug}`);
    if (!idB) errors.push(`FRUIT-001: missing canonical entity ${rule.partner}`);
    if (idA && idB) {
      if (idA === idB) {
        errors.push(`FRUIT-001: ${rule.slug} and ${rule.partner} collapsed to same canonical ID`);
      }
      const refA = byId[idA];
      const refB = byId[idB];
      if (refA && refA.parent_group !== rule.group) {
        errors.push(`FRUIT-001: ${rule.slug} must be in ${rule.group}, got ${refA.parent_group}`);
      }
      if (refB && refB.parent_group !== rule.partnerGroup) {
        errors.push(
          `FRUIT-001: ${rule.partner} must be in ${rule.partnerGroup}, got ${refB.parent_group}`
        );
      }
    }
  }

  if (!errors.some((e) => e.startsWith("FRUIT-001") || e.includes("Cross-domain"))) passed += 1;
  checks.fruit001_ownership = passed;
  return passed;
}
...
function validateDeterminism(loaded, errors, checks) {
  ...
  const rebuilt = compileFruitRuntime(catalog);
...
function main() {
  ...
  const bootstrap = loaded["fruit-bootstrap.json"];
  const categories = loaded["fruit-categories.json"];
  const groups = loaded["fruit-groups.json"];
  const indexes = loaded["fruit-indexes.json"];
  ...
  validateFruit001Ownership({ byId, bySlug, errors, checks });
  ...
  const report = {
    ...
    metrics: {
      ...
      "FRUIT-001 ownership checks": checks.fruit001_ownership,
