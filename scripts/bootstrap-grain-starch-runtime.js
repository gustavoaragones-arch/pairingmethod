#!/usr/bin/env node
/**
 * FOOD-08C.1 — Bootstrap grain & starch catalog into runtime indexes.
 * Read-only compiler: never modifies data/grain-starch-catalog.json.
 *
 * Run: node scripts/bootstrap-grain-starch-runtime.js
 */
...
const REPORT_PATH = path.join(ROOT, "reports/grain-starch-runtime-bootstrap-report.json");
...
const ARTIFACTS = [
  "grain-starch-bootstrap.json",
  "grain-starch-categories.json",
  "grain-starch-groups.json",
  "grain-starch-indexes.json",
];
...
function grainStarchRef(grainStarch) {
  return {
    id: grainStarch.id,
    slug: grainStarch.slug,
    display_name: grainStarch.display_name,
    parent_group: grainStarch.parent_group,
    parent_category: grainStarch.parent_category,
    scientific_name: grainStarch.scientific_name,
    culinary_group: grainStarch.culinary_group,
    usage_intensity: grainStarch.usage_intensity,
    origin_context: grainStarch.origin_context ?? "",
    catalog_version: grainStarch.catalog_version,
    food_ontology_version: grainStarch.food_ontology_version,
  };
}

function fieldValue(grainStarch, field) {
  if (field === "origin_context") return grainStarch.origin_context ?? "";
  return grainStarch[field];
}
...
function validateForBootstrap(catalog) {
  ...
  for (const grainStarch of catalog.grain_starches) {
    if (ids.has(grainStarch.id)) errors.push(`Duplicate id: ${grainStarch.id}`);
    ids.add(grainStarch.id);
    if (slugs.has(grainStarch.slug)) errors.push(`Duplicate slug: ${grainStarch.slug}`);
    slugs.add(grainStarch.slug);
    if (!groupSlugs.has(grainStarch.parent_group)) {
      errors.push(`Broken hierarchy: ${grainStarch.slug} parent_group ${grainStarch.parent_group}`);
    }
    if (grainStarch.parent_category !== "grain-starch") {
      errors.push(`${grainStarch.slug}: parent_category must be grain-starch`);
    }

    for (const field of VOCAB_FIELDS) {
      const val = fieldValue(grainStarch, field);
      if (val === undefined || val === null) {
        errors.push(`${grainStarch.slug}: missing ${field}`);
      } else if (VOCAB[field] && !VOCAB[field].has(val)) {
        errors.push(`${grainStarch.slug}: invalid ${field} "${val}"`);
      }
    }

    if (!grainStarch.scientific_name || !grainStarch.scientific_name.includes(" ")) {
      errors.push(`${grainStarch.slug}: invalid or missing scientific_name`);
    }

    for (const field of RESERVED_PROFILE_FIELDS) {
      if (!Array.isArray(grainStarch[field]) || grainStarch[field].length > 0) {
        errors.push(`${grainStarch.slug}: ${field} must be empty array in FOOD-08C`);
      }
    }

    if (!grainStarch.external_ids || typeof grainStarch.external_ids !== "object") {
      errors.push(`${grainStarch.slug}: missing external_ids object`);
    }
  }
  ...
  for (const g of catalog.groups) {
    ...
    const grainStarches = catalog.grain_starches
      .filter((h) => h.parent_group === g.slug)
      .map((f) => f.slug);
    for (const s of g.child_slugs) {
      if (!grainStarches.includes(s)) errors.push(`Group ${g.slug} missing grain_starch ${s}`);
    }
  }
...
function buildVocabularyIndex(grainStarches) {
  ...
  for (const grainStarch of grainStarches) {
    for (const field of VOCAB_FIELDS) {
      ...
      index[field][key].push(grainStarch.id);
    }
  }
  for (const field of VOCAB_FIELDS) {
...
function buildNamespaceLookup(categories, groups, grainStarches) {
  const byNamespace = {
    "food.grain": categories.map((c) => c.id).sort(),
  };
  for (const group of groups) {
    byNamespace[group.id] = grainStarches
...
function bootstrap(catalog) {
  const grainStarches = [...catalog.grain_starches].sort((a, b) => a.id.localeCompare(b.id));
  ...
  for (const grainStarch of grainStarches) {
    byId[grainStarch.id] = grainStarchRef(grainStarch);
    bySlug[grainStarch.slug] = grainStarch.id;

    if (!byUsageIntensity[grainStarch.usage_intensity]) byUsageIntensity[grainStarch.usage_intensity] = [];
    byUsageIntensity[grainStarch.usage_intensity].push(grainStarch.id);

    if (!byGroup[grainStarch.parent_group]) byGroup[grainStarch.parent_group] = [];
    byGroup[grainStarch.parent_group].push(grainStarch.id);

    const nameKey = grainStarch.display_name.trim().toLowerCase();
    if (!byDisplayName[nameKey]) byDisplayName[nameKey] = [];
    byDisplayName[nameKey].push(grainStarch.id);

    if (grainStarch.scientific_name) {
      if (!byScientificName[grainStarch.scientific_name]) byScientificName[grainStarch.scientific_name] = [];
      byScientificName[grainStarch.scientific_name].push(grainStarch.id);
    }
  }
  ...
  for (const grainStarch of grainStarches) {
    grainStarchToGroup[grainStarch.slug] = grainStarch.parent_group;
  }
  ...
  const groupsOut = groups.map((group) => ({
    ...
    grain_starch_slugs: [...group.child_slugs].sort(),
    grain_starch_ids: group.child_slugs.map((s) => bySlug[s]).sort(),
  }));

  const vocabularyIndex = buildVocabularyIndex(grainStarches);
  const byNamespace = buildNamespaceLookup(categories, groups, grainStarches);

  const bootstrapDoc = {
    meta: {
      domain: "grain-starch",
      source: "data/grain-starch-catalog.json",
      ...
      runtime_stability_levels: {
        ...
        level_2_intrinsic_similarity: "shares_usage_intensity, shares_scientific_name — FOOD-08C.3",
        level_3_editorial: "reserved FOOD-08D — not generated in runtime bootstrap",
      },
    },
    hierarchy: {
      category_to_groups: categoryToGroups,
      group_to_category: groupToCategory,
      group_to_grain_starches: groupToGrainStarches,
      grain_starch_to_group: grainStarchToGroup,
    },
  };
  ...
  return {
    ...
    stats: {
      ...
      grain_starches: grainStarches.length,
...
function main() {
  ...
  const report = {
    ...
    metrics: {
      ...
      "Grain & starch entities indexed": built.stats.grain_starches,
...
export {
  ...
  sortKeysDeep,
  serializeRuntime,
  bootstrap as compileGrainStarchRuntime,
};
