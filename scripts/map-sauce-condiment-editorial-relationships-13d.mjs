#!/usr/bin/env node
/**
 * FOOD-13D — Editorial relationship mapper for sauce-condiment ontology.
 * Level 3 (editorial) relationships — human-curated, evidence-backed.
 * Does not modify catalog, runtime graph, or structural relationship layer.
 *
 * Run: node scripts/map-sauce-condiment-editorial-relationships-13d.mjs
 */

import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { serializeRuntime } from "./build-sauce-condiment-runtime-13c.js";
import { EDITORIAL_CURATED, FORWARD_REFERENCE_IDS } from "./sauce-condiment-editorial-seed-13d.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_PATH = path.join(ROOT, "data/runtime/sauce-condiment-runtime.json");
const OUTPUT_PATH = path.join(ROOT, "data/runtime/sauce-condiment-editorial-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/sauce-condiment-editorial-report.json");
const EDGE_VERSION = "1.0";

const EDITORIAL_RELATIONSHIP_TYPES = [
  "similar_to",
  "substitutes_for",
  "commonly_served_with",
  "similar_sauce_condiments",
];

const SYMMETRIC_TYPES = new Set(["similar_to", "similar_sauce_condiments"]);

const EDITORIAL_TIERS = {
  A: {
    label: "high_confidence_culinary_equivalence",
    relationships: ["similar_to", "substitutes_for", "similar_sauce_condiments"],
  },
  B: {
    label: "common_culinary_association",
    relationships: ["commonly_served_with"],
    scope: "intra_domain",
  },
  C: {
    label: "cross_domain_forward_reference",
    relationships: ["commonly_served_with"],
    scope: "forward_reference",
  },
};

const VALID_EDITORIAL_REVIEW = new Set(["approved", "pending"]);

const TIER_COUNT_TARGETS = {
  A: { min: 35, max: 55 },
  B: { min: 70, max: 95 },
  C: { min: 110, max: 160 },
  total: { min: 220, max: 300 },
};

const SAUCE001_FORBIDDEN_EDITORIAL_PAIRS = [
  ["bechamel", "sauce-mornay"],
  ["veloute", "sauce-mornay"],
  ["espagnole", "demi-glace"],
  ["tomato-mother-sauce", "marinara-sauce"],
];

const CROSS_DOMAIN_FORBIDDEN_SLUGS = [
  "miso",
  "natto",
  "hummus",
  "tahini",
  "peanut-butter",
  "garam-masala",
  "curry-powder",
  "mustard-seed",
  "tomato",
  "garlic",
  "cane-sugar",
  "honey",
];

function sortKeysDeep(value) {
  if (Array.isArray(value)) return value.map(sortKeysDeep);
  if (value && typeof value === "object") {
    return Object.keys(value)
      .sort()
      .reduce((acc, key) => {
        acc[key] = sortKeysDeep(value[key]);
        return acc;
      }, {});
  }
  return value;
}

function writeJson(filePath, data) {
  const text = `${JSON.stringify(sortKeysDeep(data), null, 2)}\n`;
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, text, "utf8");
  return text;
}

function edgeKey(edge) {
  return `${edge.source}\t${edge.relationship}\t${edge.target}`;
}

function validateSauceEditorialRule(entries) {
  const errors = [];
  for (const entry of entries) {
    if (!["similar_to", "substitutes_for", "similar_sauce_condiments"].includes(entry.relationship)) {
      continue;
    }
    for (const [slugA, slugB] of SAUCE001_FORBIDDEN_EDITORIAL_PAIRS) {
      const matches =
        (entry.source === slugA && entry.target === slugB) ||
        (entry.source === slugB && entry.target === slugA);
      if (matches) {
        errors.push(
          `SAUCE-001 Editorial Rule: ${entry.relationship} forbidden between ${slugA} and ${slugB}`
        );
      }
    }
  }
  return errors;
}

function validateSauceEditorialEdges(edges, byId) {
  const errors = [];
  for (const edge of edges) {
    if (!["similar_to", "substitutes_for", "similar_sauce_condiments"].includes(edge.relationship)) {
      continue;
    }
    const sourceSlug = byId[edge.source]?.slug;
    const targetSlug = byId[edge.target]?.slug;
    if (!sourceSlug || !targetSlug) continue;
    for (const [slugA, slugB] of SAUCE001_FORBIDDEN_EDITORIAL_PAIRS) {
      const matches =
        (sourceSlug === slugA && targetSlug === slugB) ||
        (sourceSlug === slugB && targetSlug === slugA);
      if (matches) {
        errors.push(
          `SAUCE-001 Editorial Rule: resolved edge ${edge.relationship} ${sourceSlug} <-> ${targetSlug}`
        );
      }
    }
  }
  return errors;
}

function loadInputs() {
  const runtime = JSON.parse(fs.readFileSync(RUNTIME_PATH, "utf8"));
  return {
    runtime,
    slugMap: runtime.indexes.by_slug,
    structural: runtime.relationships,
    catalogVersion: runtime.meta.catalog_version,
  };
}

function resolveSauceCondimentId(slugMap, slug) {
  return slugMap[slug] ?? null;
}

function canonicalPair(source, target) {
  return source.localeCompare(target) <= 0 ? [source, target] : [target, source];
}

function isForwardReference(id) {
  return FORWARD_REFERENCE_IDS.has(id);
}

function isCrossDomainTarget(target) {
  return target.startsWith("food.") && !target.startsWith("food.sauce-condiment.");
}

export function mapSauceCondimentEditorialRelationships({ slugMap, catalogVersion }) {
  const seedErrors = validateSauceEditorialRule(EDITORIAL_CURATED);
  if (seedErrors.length) {
    throw new Error(seedErrors.join("\n"));
  }

  const sauceCondimentIds = new Set(Object.values(slugMap));
  const edges = [];
  const seen = new Set();
  const forwardReferencesUsed = new Set();
  const tierCounts = { A: 0, B: 0, C: 0 };

  for (const entry of EDITORIAL_CURATED) {
    const sourceId = resolveSauceCondimentId(slugMap, entry.source);
    if (!sourceId) {
      throw new Error(`Unknown source slug: ${entry.source}`);
    }

    let targetId = entry.target;
    if (isCrossDomainTarget(entry.target)) {
      if (!isForwardReference(entry.target)) {
        throw new Error(`Undocumented cross-domain target: ${entry.target}`);
      }
      forwardReferencesUsed.add(entry.target);
      targetId = entry.target;
    } else {
      targetId = resolveSauceCondimentId(slugMap, entry.target);
      if (!targetId) {
        throw new Error(`Unknown target slug: ${entry.target}`);
      }
    }

    let source = sourceId;
    let target = targetId;

    if (SYMMETRIC_TYPES.has(entry.relationship)) {
      [source, target] = canonicalPair(sourceId, targetId);
    }

    const edge = {
      source,
      relationship: entry.relationship,
      target,
      confidence: entry.confidence,
      derived_from: "editorial",
      stability_level: "editorial",
      editorial_tier: entry.editorial_tier,
      editorial_review: entry.editorial_review,
      evidence: entry.evidence,
      version: EDGE_VERSION,
    };

    const key = edgeKey(edge);
    if (seen.has(key)) continue;
    seen.add(key);
    edges.push(edge);
    tierCounts[entry.editorial_tier] = (tierCounts[entry.editorial_tier] ?? 0) + 1;
  }

  edges.sort((a, b) => edgeKey(a).localeCompare(edgeKey(b)));

  const relationshipTypeCounts = Object.fromEntries(
    EDITORIAL_RELATIONSHIP_TYPES.map((type) => [type, 0])
  );
  for (const edge of edges) {
    relationshipTypeCounts[edge.relationship] = (relationshipTypeCounts[edge.relationship] ?? 0) + 1;
  }

  const usedTypes = EDITORIAL_RELATIONSHIP_TYPES.filter((type) => relationshipTypeCounts[type] > 0);

  return {
    meta: {
      phase: "FOOD-13D",
      domain: "sauce-condiment",
      version: EDGE_VERSION,
      catalog_version: catalogVersion ?? null,
      layer: "editorial",
      stability_level: "editorial",
      runtime_stability_level: 3,
      editorial_tiers: EDITORIAL_TIERS,
      relationship_types: usedTypes,
      edge_count: edges.length,
      tier_counts: tierCounts,
      forward_references: [...forwardReferencesUsed].sort(),
      inputs: [
        "data/sauce-condiment-catalog.json",
        "data/runtime/sauce-condiment-runtime.json",
        "scripts/sauce-condiment-editorial-seed-13d.js",
      ],
      scope_exclusions: [
        "wine_pairings",
        "runtime_structural_edges",
        "popularity_rankings",
        "preparation_advice",
        "recipes",
        "finished_dishes",
        "confectionery_products",
        "nutrition",
        "health_claims",
      ],
      association_rule:
        "Editorial edges describe ingredient compatibility, not dish composition or finished dishes.",
      sauce001_editorial_rule:
        "SAUCE-001: established culinary identities required — mother sauces and direct classical derivatives must not be editorially interchangeable in Tier A.",
      sauce002_editorial_rule:
        "SAUCE-002: component ingredients owned by other domains — cross-domain composition references use canonical IDs only in Tier C.",
    },
    edges,
    stats: {
      relationship_types: usedTypes.length,
      total_edges: edges.length,
      tier_counts: tierCounts,
      relationship_type_counts: relationshipTypeCounts,
      forward_reference_count: forwardReferencesUsed.size,
    },
    sauceCondimentIds,
  };
}

function validateReciprocity(edges, byId) {
  const errors = [];
  const symmetricKeys = new Set();

  for (const edge of edges) {
    if (!SYMMETRIC_TYPES.has(edge.relationship)) continue;
    const sourceSlug = byId[edge.source]?.slug;
    const targetSlug = byId[edge.target]?.slug;
    if (!sourceSlug || !targetSlug) continue;
    const [a, b] = [sourceSlug, targetSlug].sort();
    const pairKey = `${edge.relationship}\t${a}\t${b}`;
    if (symmetricKeys.has(pairKey)) {
      errors.push(`Reciprocity: duplicate symmetric pair ${edge.relationship} ${a} <-> ${b}`);
    }
    symmetricKeys.add(pairKey);
  }

  return errors;
}

function validateNodeCoverage(edges, sauceCondimentIds, byId) {
  const errors = [];
  const connected = new Set();

  for (const edge of edges) {
    if (sauceCondimentIds.has(edge.source)) connected.add(edge.source);
    if (sauceCondimentIds.has(edge.target)) connected.add(edge.target);
  }

  const orphanSlugs = [];
  for (const id of sauceCondimentIds) {
    if (!connected.has(id)) {
      orphanSlugs.push(byId[id]?.slug ?? id);
    }
  }

  if (orphanSlugs.length > 0) {
    errors.push(`Orphan detection: ${orphanSlugs.length} entities without editorial edges`);
  }

  return { errors, orphanCount: orphanSlugs.length, connectedCount: connected.size };
}

function validateEditorial(output, structural, sauceCondimentIds, slugMap) {
  const errors = [];
  const seen = new Set();
  const structuralKeys = new Set(structural.edges.map(edgeKey));
  let duplicates = 0;
  let missingEntities = 0;
  let orphanForwardRefs = 0;
  let conflicts = 0;
  let pendingReview = 0;

  for (const slug of CROSS_DOMAIN_FORBIDDEN_SLUGS) {
    if (slugMap[slug]) {
      errors.push(`Cross-domain: forbidden sauce-condiment runtime slug ${slug}`);
    }
  }

  for (const edge of output.edges) {
    const key = edgeKey(edge);
    if (seen.has(key)) {
      duplicates += 1;
      errors.push(`Duplicate editorial edge: ${key}`);
    }
    seen.add(key);

    if (edge.source === edge.target) {
      errors.push(`Self-reference: ${key}`);
    }

    if (!sauceCondimentIds.has(edge.source)) {
      missingEntities += 1;
      errors.push(`Missing source entity: ${edge.source}`);
    }

    if (isCrossDomainTarget(edge.target)) {
      if (!FORWARD_REFERENCE_IDS.has(edge.target)) {
        orphanForwardRefs += 1;
        errors.push(`Undocumented forward reference: ${edge.target}`);
      }
    } else if (!sauceCondimentIds.has(edge.target)) {
      missingEntities += 1;
      errors.push(`Missing target entity: ${edge.target}`);
    }

    if (structuralKeys.has(key)) {
      conflicts += 1;
      errors.push(`Conflicts with structural relationship: ${key}`);
    }

    if (edge.derived_from !== "editorial") {
      errors.push(`Invalid derived_from on ${key}`);
    }
    if (edge.stability_level !== "editorial") {
      errors.push(`Invalid stability_level on ${key}`);
    }
    if (edge.confidence !== "high") {
      errors.push(`Invalid confidence on ${key}`);
    }
    if (!edge.evidence || !edge.evidence.trim()) {
      errors.push(`Missing evidence on ${key}`);
    }
    if (!VALID_EDITORIAL_REVIEW.has(edge.editorial_review)) {
      errors.push(`Invalid editorial_review on ${key}`);
    }
    if (edge.editorial_review === "pending") {
      pendingReview += 1;
    }
    if (!["A", "B", "C"].includes(edge.editorial_tier)) {
      errors.push(`Invalid editorial_tier on ${key}`);
    }
    if (!EDITORIAL_RELATIONSHIP_TYPES.includes(edge.relationship)) {
      errors.push(`Unknown relationship type: ${edge.relationship}`);
    }

    if (
      edge.editorial_tier === "A" &&
      !["similar_to", "substitutes_for", "similar_sauce_condiments"].includes(edge.relationship)
    ) {
      errors.push(`Tier A relationship mismatch: ${key}`);
    }
    if (edge.editorial_tier === "B" && edge.relationship !== "commonly_served_with") {
      errors.push(`Tier B relationship mismatch: ${key}`);
    }
    if (edge.editorial_tier === "B" && isCrossDomainTarget(edge.target)) {
      errors.push(`Tier B must be intra-domain: ${key}`);
    }
    if (edge.editorial_tier === "C" && edge.relationship !== "commonly_served_with") {
      errors.push(`Tier C relationship mismatch: ${key}`);
    }
    if (edge.editorial_tier === "C" && !isCrossDomainTarget(edge.target)) {
      errors.push(`Tier C must be cross-domain forward reference: ${key}`);
    }
  }

  for (const [tier, target] of Object.entries(TIER_COUNT_TARGETS)) {
    if (tier === "total") {
      if (output.edges.length < target.min || output.edges.length > target.max) {
        errors.push(
          `Edge count ${output.edges.length} outside FOOD-13D target ${target.min}-${target.max}`
        );
      }
      continue;
    }
    const count = output.stats.tier_counts[tier] ?? 0;
    if (count < target.min || count > target.max) {
      errors.push(`${tier}: ${count} edges outside FOOD-13D target ${target.min}-${target.max}`);
    }
  }

  return { errors, duplicates, missingEntities, orphanForwardRefs, conflicts, pendingReview };
}

function main() {
  const { slugMap, structural, catalogVersion } = loadInputs();
  const output = mapSauceCondimentEditorialRelationships({ slugMap, catalogVersion });
  const byIdForGovernance = Object.fromEntries(
    Object.entries(slugMap).map(([slug, id]) => [id, { slug, id }])
  );
  const validation = validateEditorial(output, structural, output.sauceCondimentIds, slugMap);
  validation.errors.push(...validateSauceEditorialEdges(output.edges, byIdForGovernance));
  validation.errors.push(...validateReciprocity(output.edges, byIdForGovernance));
  const coverage = validateNodeCoverage(output.edges, output.sauceCondimentIds, byIdForGovernance);
  validation.errors.push(...coverage.errors);

  const rebuilt = mapSauceCondimentEditorialRelationships({ slugMap, catalogVersion });
  const determinismPass =
    serializeRuntime({ meta: output.meta, edges: output.edges }) ===
    serializeRuntime({ meta: rebuilt.meta, edges: rebuilt.edges });

  const overall = validation.errors.length === 0 && determinismPass ? "PASS" : "FAIL";

  if (overall === "FAIL") {
    const report = {
      phase: "FOOD-13D",
      domain: "sauce-condiment",
      overall_result: "FAIL",
      errors: validation.errors,
      metrics: {
        "Editorial relationship types": output.stats.relationship_types,
        "Editorial edges": output.stats.total_edges,
        "Tier A edges": output.stats.tier_counts.A,
        "Tier B edges": output.stats.tier_counts.B,
        "Tier C edges": output.stats.tier_counts.C,
        "Duplicate edges": validation.duplicates,
        "Missing entities": validation.missingEntities,
        "Forward reference orphans": validation.orphanForwardRefs,
        Conflicts: validation.conflicts,
        "Connected entities": coverage.connectedCount,
        "Orphan entities": coverage.orphanCount,
        Determinism: determinismPass ? "PASS" : "FAIL",
        "Overall result": "FAIL",
      },
    };
    writeJson(REPORT_PATH, report);
    console.error(validation.errors.join("\n"));
    if (!determinismPass) console.error("Determinism check failed");
    process.exit(1);
  }

  writeJson(OUTPUT_PATH, { meta: output.meta, edges: output.edges });

  const report = {
    phase: "FOOD-13D",
    domain: "sauce-condiment",
    catalog_version: output.meta.catalog_version,
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/sauce-condiment-editorial-relationships.json",
    forward_references: output.meta.forward_references,
    metrics: {
      "Editorial relationship types": output.stats.relationship_types,
      "Editorial edges": output.stats.total_edges,
      "Tier A edges": output.stats.tier_counts.A,
      "Tier B edges": output.stats.tier_counts.B,
      "Tier C edges": output.stats.tier_counts.C,
      "Pending review edges": validation.pendingReview,
      "Duplicate edges": 0,
      "Missing entities": 0,
      "Forward reference orphans": 0,
      Conflicts: 0,
      "Connected entities": coverage.connectedCount,
      "Orphan entities": coverage.orphanCount,
      "Reciprocity validation": "PASS",
      Determinism: "PASS",
      "Deterministic ordering": "PASS",
      "Overall result": "PASS",
    },
    relationship_type_counts: output.stats.relationship_type_counts,
    editorial_tiers: EDITORIAL_TIERS,
    association_rule: output.meta.association_rule,
    sauce001_editorial_rule: output.meta.sauce001_editorial_rule,
    sauce002_editorial_rule: output.meta.sauce002_editorial_rule,
  };

  writeJson(REPORT_PATH, report);
  console.log(JSON.stringify(report.metrics, null, 2));
  console.log(`Editorial relationships: ${OUTPUT_PATH}`);
  console.log(`Report: ${REPORT_PATH}`);
}

const isMain =
  process.argv[1] &&
  path.resolve(fileURLToPath(import.meta.url)) === path.resolve(process.argv[1]);

if (isMain) {
  main();
}
