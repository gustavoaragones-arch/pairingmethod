#!/usr/bin/env node
/**
 * FOOD-06C.3 — Structural relationship mapper for fungi ontology.
 * Level 1 (structural) + Level 2 (intrinsic similarity) only — no editorial edges.
 *
 * Run: node scripts/map-fungi-relationships-06c.mjs
 */
...
import { ARTIFACTS, serializeRuntime } from "./bootstrap-fungi-runtime.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const RELATIONSHIPS_PATH = path.join(RUNTIME_DIR, "fungi-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/fungi-relationship-mapper-report.json");
...
function requireRuntimeValidation() {
  const result = spawnSync("node", ["scripts/validate-runtime-fungi-06c.mjs"], {
    cwd: ROOT,
    encoding: "utf8",
...
function loadRuntime() {
  ...
  return {
    bootstrap: loaded["fungi-bootstrap.json"],
    categories: loaded["fungi-categories.json"],
    groups: loaded["fungi-groups.json"],
    indexes: loaded["fungi-indexes.json"],
    byId: loaded["fungi-indexes.json"].by_id,
  };
}
...
function groupFungiByField(fungi, field) {
  const buckets = {};
  for (const fungus of fungi) {
    const value = fungus[field] ?? "";
    if (value === "" || value === "none") continue;
    if (!buckets[value]) buckets[value] = [];
    buckets[value].push(fungus.id);
  }
  return buckets;
}

export function mapFungiRelationships(runtime) {
  ...
  const fungi = Object.values(byId);

  for (const fungus of fungi) {
    const group = groupBySlug[fungus.parent_group];
    if (!group) continue;

    addEdge(edges, seen, makeEdge(fungus.id, "belongs_to_group", group.id, "structural"));

    const category = categoryBySlug[group.parent_category];
    if (category) {
      addEdge(edges, seen, makeEdge(fungus.id, "belongs_to_category", category.id, "structural"));
    }
  }
  ...
  for (const group of groups) {
    ...
    for (const fungusId of group.fungus_ids) {
      addEdge(edges, seen, makeEdge(group.id, "group_contains", fungusId, "structural"));
    }
  }

  for (const [relationship, field] of Object.entries(METADATA_FIELDS)) {
    const buckets = groupFungiByField(fungi, field);
...
function validateRelationships(output, entityIds) {
  ...
  for (const edge of output.edges) {
    ...
    if (FORBIDDEN_EDITORIAL_TYPES.has(edge.relationship)) {
      errors.push(`Editorial relationship forbidden in FOOD-06C: ${edge.relationship}`);
    }
...
function main() {
  ...
  const output = mapFungiRelationships(runtime);

  const validation = validateRelationships(output, entityIds);
  const determinismPass =
    serializeRuntime(output) === serializeRuntime(mapFungiRelationships(runtime));
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/fungi-relationships.json",
    metrics: {
      "Relationship types": output.stats.relationship_types,
      "Total edges": output.stats.total_edges,
      "Level 1 structural edges": output.stats.level_1_structural_edges,
