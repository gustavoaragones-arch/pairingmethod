#!/usr/bin/env node
/**
 * FOOD-12C — Runtime audit for consolidated sweet-flavor runtime graph.
 * Structural verification only: no catalog writes, no repairs, no editorial edges.
 *
 * Run: node scripts/runtime-audit-sweet-flavor-12c.mjs
 * Output: reports/sweet-flavor-runtime-report.json
 */
...
import {
  ...
  fieldValue,
  serializeRuntime,
  compileSweetFlavorRuntime,
  ALL_RELATIONSHIP_TYPES,
  RUNTIME_STABILITY_LEVELS,
  RUNTIME_PATH,
  CATALOG_PATH,
} from "./build-sweet-flavor-runtime-12c.js";
...
function loadRuntime() {
  if (!fs.existsSync(RUNTIME_PATH)) {
    throw new Error("Missing runtime artifact: data/runtime/sweet-flavor-runtime.json");
  }
...
function validateHierarchy({ hierarchy, categories, groups, bySlug, byId, errors, checks }) {
  ...
  const { category_to_groups, group_to_category, group_to_sweet_flavors, sweet_flavor_to_group } =
    hierarchy;
  ...
  for (const group of groups) {
    for (const entitySlug of group.sweet_flavor_slugs) {
      if (!bySlug[entitySlug]) {
        errors.push(`Hierarchy: group ${group.slug} references missing entity slug ${entitySlug}`);
      }
    }
    const expectedIds = group.sweet_flavor_slugs.map((s) => bySlug[s]).filter(Boolean).sort();
    if (JSON.stringify(group.sweet_flavor_ids) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} sweet_flavor_ids mismatch sweet_flavor_slugs`);
    }
    const indexedEntities = group_to_sweet_flavors[group.slug] ?? [];
  ...
  for (const [entitySlug, parentGroup] of Object.entries(sweet_flavor_to_group)) {
    if (!bySlug[entitySlug]) {
      errors.push(`Hierarchy: sweet_flavor_to_group references unknown slug ${entitySlug}`);
      continue;
    }
    ...
    if (group && !group.sweet_flavor_slugs.includes(entitySlug)) {
      errors.push(
        `Hierarchy: reciprocal failure — ${entitySlug} not in group ${parentGroup} sweet_flavor_slugs`
      );
    }
  }
  if (
...
function validateByGroup({ byGroup, byId, groups, errors, checks }) {
  ...
  for (const group of groups) {
    const expected = group.sweet_flavor_ids.sort();
    const indexed = (byGroup[group.slug] ?? []).slice().sort();
    if (JSON.stringify(indexed) !== JSON.stringify(expected)) {
      errors.push(`By group: ${group.slug} index mismatch with group sweet_flavor_ids`);
    }
  }
...
function validateNamespace({ byNamespace, categories, groups, byId, errors, checks }) {
  ...
  const nsCategory = byNamespace["food.sweet-flavor"] ?? [];

  if (JSON.stringify(nsCategory) !== JSON.stringify(categoryIds.sort())) {
    errors.push("Namespace: food.sweet-flavor namespace mismatch with categories");
  } else passed += 1;
  ...
  for (const group of groups) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      errors.push(`Namespace: ${group.id} namespace mismatch with group sweet_flavor_ids`);
    }
  }
...
function validateCatalogCoverage({ byId, entityCount, errors, checks }) {
  ...
  const catalogIds = new Set((catalog.sweet_flavors ?? []).map((e) => e.id));
...
function validateSweetFlavorGovernance({ byId, bySlug, errors, checks }) {
  let passed = 0;
  ...
  for (const slug of FOOD13_FORBIDDEN_SLUGS) {
    if (bySlug[slug]) {
      errors.push(`FOOD13-001: deferred condiment slug ${slug} must not be a runtime entity`);
    }
  }
  ...
  for (const slug of HONEY_GROUP_FORBIDDEN) {
    if (bySlug[slug]) {
      errors.push(`HONEY-001: supplement bee product ${slug} must not be a runtime entity`);
    }
    const id = bySlug[slug];
    if (id && byId[id]?.parent_group === "honey-bee-products") {
      errors.push(`HONEY-001: ${slug} must not be in honey-bee-products group`);
    }
  }

  for (const rule of COCOA001_REQUIRED) {
    const id = bySlug[rule.slug];
    if (!id) {
      errors.push(`COCOA-001: missing required entity ${rule.slug}`);
      continue;
    }
    const ref = byId[id];
    if (ref && ref.parent_group !== rule.parent_group) {
      errors.push(
        `COCOA-001: ${rule.slug} must be in ${rule.parent_group}, got ${ref.parent_group}`
      );
    }
  }

  for (const rule of SWEET001_SEPARATE) {
    const id = bySlug[rule.slug];
    if (!id) {
      errors.push(`SWEET-001: missing separate entity ${rule.slug}`);
      continue;
    }
    const ref = byId[id];
    if (ref && ref.parent_group !== rule.parent_group) {
      errors.push(
        `SWEET-001: ${rule.slug} must be in ${rule.parent_group}, got ${ref.parent_group}`
      );
    }
  }
  ...
  if (
    !errors.some(
      (e) =>
        e.startsWith("SWEET-001") ||
        e.startsWith("COCOA-001") ||
        e.startsWith("HONEY-001") ||
        e.startsWith("FOOD13-001") ||
  ...
  checks.sweet_flavor_governance = passed;
  return passed;
}
...
function validateDeterminism(runtime, errors, checks) {
  let passed = 0;
  const catalog = JSON.parse(fs.readFileSync(CATALOG_PATH, "utf8"));
  const rebuilt = compileSweetFlavorRuntime(catalog);
  const expected = serializeRuntime(runtime);
  const actual = serializeRuntime(rebuilt);

  if (expected !== actual) {
    errors.push("Determinism: sweet-flavor-runtime.json differs from catalog recompilation");
  } else {
    passed += 1;
  }
...
function main() {
  ...
  validateSweetFlavorGovernance({ byId, bySlug, errors, checks });
  ...
  const report = {
    ...
    metrics: {
      ...
      "Sweet flavor governance checks": checks.sweet_flavor_governance,
