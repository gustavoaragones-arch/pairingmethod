#!/usr/bin/env node
/**
 * FOOD-10C.2 — Runtime validator for nut-seed bootstrap artifacts.
 * Structural verification only: no catalog writes, no repairs, no relationships.
 *
 * Run: node scripts/validate-runtime-nut-seed-10c.mjs
 * Output: reports/nut-seed-runtime-validator-report.json
 */
...
import {
  ...
  fieldValue,
  serializeRuntime,
  compileNutSeedRuntime,
} from "./bootstrap-nut-seed-runtime.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const CATALOG_PATH = path.join(ROOT, "data/nut-seed-catalog.json");
const REPORT_PATH = path.join(ROOT, "reports/nut-seed-runtime-validator-report.json");

const ARTIFACT_BUILDERS = {
  "nut-seed-bootstrap.json": (built) => built.bootstrapDoc,
  "nut-seed-categories.json": (built) => built.categoriesOut,
  "nut-seed-groups.json": (built) => built.groupsOut,
  "nut-seed-indexes.json": (built) => built.indexesDoc,
};

const NUT001_ASSIGNMENTS = [
  { slug: "peanut", group: "peanuts" },
  { slug: "almond", group: "tree-nuts" },
  { slug: "sesame", group: "edible-seeds" },
  { slug: "pumpkin-seed", group: "edible-seeds" },
  { slug: "sunflower-seed", group: "edible-seeds" },
];

const NUT002_SEPARATED = [
  { slug: "almond", group: "tree-nuts", partner: "almond-flour", partnerGroup: "nut-products" },
  { slug: "almond", group: "tree-nuts", partner: "almond-butter", partnerGroup: "nut-products" },
  { slug: "peanut", group: "peanuts", partner: "peanut-butter", partnerGroup: "nut-products" },
  { slug: "sesame", group: "edible-seeds", partner: "tahini", partnerGroup: "seed-products" },
  {
    slug: "hazelnut",
    group: "tree-nuts",
    partner: "hazelnut-flour",
    partnerGroup: "nut-products",
  },
];
...
function validateHierarchy({ bootstrap, categories, groups, bySlug, byId, errors, checks }) {
  ...
  const { category_to_groups, group_to_category, group_to_nut_seeds, nut_seed_to_group } =
    bootstrap.hierarchy;
  ...
  for (const group of groups) {
    for (const entitySlug of group.nut_seed_slugs) {
      if (!bySlug[entitySlug]) {
        errors.push(`Hierarchy: group ${group.slug} references missing entity slug ${entitySlug}`);
      }
    }
    const expectedIds = group.nut_seed_slugs.map((s) => bySlug[s]).filter(Boolean).sort();
    if (JSON.stringify(group.nut_seed_ids) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} nut_seed_ids mismatch nut_seed_slugs`);
    }
    const indexedEntities = group_to_nut_seeds[group.slug] ?? [];
    if (JSON.stringify(indexedEntities) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} bootstrap entity list mismatch`);
    }
  ...
  for (const [entitySlug, parentGroup] of Object.entries(nut_seed_to_group)) {
    if (!bySlug[entitySlug]) {
      errors.push(`Hierarchy: nut_seed_to_group references unknown slug ${entitySlug}`);
      continue;
    }
    if (!groupSlugs.has(parentGroup)) {
      errors.push(`Hierarchy: entity ${entitySlug} parent group ${parentGroup} missing`);
      continue;
    }
    ...
    if (group && !group.nut_seed_slugs.includes(entitySlug)) {
      errors.push(
        `Hierarchy: reciprocal failure — ${entitySlug} not in group ${parentGroup} nut_seed_slugs`
      );
    }
  }
  if (
...
function validateByGroup({ byGroup, byId, groups, errors, checks }) {
  ...
  for (const group of groups) {
    const expected = group.nut_seed_ids.sort();
    const indexed = (byGroup[group.slug] ?? []).slice().sort();
    if (JSON.stringify(indexed) !== JSON.stringify(expected)) {
      errors.push(`By group: ${group.slug} index mismatch with group nut_seed_ids`);
    }
  }
...
function validateCrossIndex({
  ...
  for (const group of groups) {
    for (const id of group.nut_seed_ids) {
      const ref = byId[id];
      if (!ref) {
        errors.push(`Cross-index: group ${group.slug} nut_seed_id ${id} missing from by_id`);
        continue;
      }
      if (bySlug[ref.slug] !== id) errors.push(`Cross-index: group entity ${id} by_slug mismatch`);
      if (bootstrap.hierarchy.nut_seed_to_group[ref.slug] !== group.slug) {
        errors.push(`Cross-index: group ${group.slug} entity ${ref.slug} hierarchy mismatch`);
      }
    }
  }
...
function validateNamespace({ byNamespace, categories, groups, byId, errors, checks }) {
  ...
  const nsCategory = byNamespace["food.nut-seed"] ?? [];

  if (JSON.stringify(nsCategory) !== JSON.stringify(categoryIds.sort())) {
    errors.push("Namespace: food.nut-seed namespace mismatch with categories");
  } else passed += 1;
  ...
  for (const group of groups) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      errors.push(`Namespace: ${group.id} namespace mismatch with group nut_seed_ids`);
    }
  }
...
function validateCatalogCoverage({ byId, bySlug, errors, checks }) {
  let passed = 0;
  const catalog = JSON.parse(fs.readFileSync(CATALOG_PATH, "utf8"));
  const catalogIds = new Set((catalog.nut_seeds ?? []).map((e) => e.id));
...
function validateNutSeedGovernance({ byId, bySlug, errors, checks }) {
  let passed = 0;
  ...
  for (const rule of NUT001_ASSIGNMENTS) {
    const id = bySlug[rule.slug];
    if (!id) {
      errors.push(`NUT-001: missing canonical entity ${rule.slug}`);
      continue;
    }
    const ref = byId[id];
    if (ref && ref.parent_group !== rule.group) {
      errors.push(`NUT-001: ${rule.slug} must be in ${rule.group}, got ${ref.parent_group}`);
    }
  }

  if (bySlug["peanut"] && byId[bySlug["peanut"]]?.parent_group === "tree-nuts") {
    errors.push("BOTAN-001: peanut must not be classified as tree-nuts");
  }

  for (const rule of NUT002_SEPARATED) {
    const idA = bySlug[rule.slug];
    const idB = bySlug[rule.partner];
    if (!idA) errors.push(`NUT-002: missing canonical entity ${rule.slug}`);
    if (!idB) errors.push(`NUT-002: missing canonical entity ${rule.partner}`);
    if (idA && idB) {
      if (idA === idB) {
        errors.push(`NUT-002: ${rule.slug} and ${rule.partner} collapsed to same canonical ID`);
      }
      const refA = byId[idA];
      const refB = byId[idB];
      if (refA && refA.parent_group !== rule.group) {
        errors.push(`NUT-002: ${rule.slug} must be in ${rule.group}, got ${refA.parent_group}`);
      }
      if (refB && refB.parent_group !== rule.partnerGroup) {
        errors.push(
          `NUT-002: ${rule.partner} must be in ${rule.partnerGroup}, got ${refB.parent_group}`
        );
      }
    }
  }
  ...
  if (
    !errors.some(
      (e) =>
        e.startsWith("NUT-001") ||
        e.startsWith("NUT-002") ||
        e.startsWith("BOTAN-001") ||
...
function validateDeterminism(loaded, errors, checks) {
  let passed = 0;
  const catalog = JSON.parse(fs.readFileSync(CATALOG_PATH, "utf8"));
  const rebuilt = compileNutSeedRuntime(catalog);
...
function main() {
  ...
  const bootstrap = loaded["nut-seed-bootstrap.json"];
  const categories = loaded["nut-seed-categories.json"];
  const groups = loaded["nut-seed-groups.json"];
  const indexes = loaded["nut-seed-indexes.json"];
  ...
  validateCatalogCoverage({ byId, bySlug, errors, checks });
  validateNutSeedGovernance({ byId, bySlug, errors, checks });
  ...
  const report = {
    ...
    metrics: {
      ...
      "NUT governance checks": checks.nut_governance,
