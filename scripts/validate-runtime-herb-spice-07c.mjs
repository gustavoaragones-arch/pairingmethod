#!/usr/bin/env node
/**
 * FOOD-07C.2 — Runtime validator for herb & spice bootstrap artifacts.
 * Structural verification only: no catalog writes, no repairs, no relationships.
 *
 * Run: node scripts/validate-runtime-herb-spice-07c.mjs
 * Output: reports/herb-spice-runtime-validator-report.json
 */
...
import {
  ...
  fieldValue,
  serializeRuntime,
  compileHerbSpiceRuntime,
} from "./bootstrap-herb-spice-runtime.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const CATALOG_PATH = path.join(ROOT, "data/herb-spice-catalog.json");
const REPORT_PATH = path.join(ROOT, "reports/herb-spice-runtime-validator-report.json");

const ARTIFACT_BUILDERS = {
  "herb-spice-bootstrap.json": (built) => built.bootstrapDoc,
  "herb-spice-categories.json": (built) => built.categoriesOut,
  "herb-spice-groups.json": (built) => built.groupsOut,
  "herb-spice-indexes.json": (built) => built.indexesDoc,
};
...
function validateHierarchy({ bootstrap, categories, groups, bySlug, byId, errors, checks }) {
  ...
  const { category_to_groups, group_to_category, group_to_herb_spices, herb_spice_to_group } = bootstrap.hierarchy;
  ...
  for (const group of groups) {
    for (const herbSpiceSlug of group.herb_spice_slugs) {
      if (!bySlug[herbSpiceSlug]) {
        errors.push(`Hierarchy: group ${group.slug} references missing herb_spice slug ${herbSpiceSlug}`);
      }
    }
    const expectedIds = group.herb_spice_slugs.map((s) => bySlug[s]).filter(Boolean).sort();
    if (JSON.stringify(group.herb_spice_ids) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} herb_spice_ids mismatch herb_spice_slugs`);
    }
    const indexedHerbSpices = group_to_herb_spices[group.slug] ?? [];
    if (JSON.stringify(indexedHerbSpices) !== JSON.stringify(expectedIds)) {
      errors.push(`Hierarchy: group ${group.slug} bootstrap herb_spice list mismatch`);
    }
  ...
  for (const [herbSpiceSlug, parentGroup] of Object.entries(herb_spice_to_group)) {
    if (!bySlug[herbSpiceSlug]) {
      errors.push(`Hierarchy: herb_spice_to_group references unknown slug ${herbSpiceSlug}`);
      continue;
    }
    if (!groupSlugs.has(parentGroup)) {
      errors.push(`Hierarchy: herb_spice ${herbSpiceSlug} parent group ${parentGroup} missing`);
      continue;
    }
    const ref = byId[bySlug[herbSpiceSlug]];
    if (ref.parent_group !== parentGroup) {
      errors.push(`Hierarchy: herb_spice ${herbSpiceSlug} parent_group ${ref.parent_group} != index ${parentGroup}`);
    }
    const group = groups.find((g) => g.slug === parentGroup);
    if (group && !group.herb_spice_slugs.includes(herbSpiceSlug)) {
      errors.push(`Hierarchy: reciprocal failure — ${herbSpiceSlug} not in group ${parentGroup} herb_spice_slugs`);
    }
  }
  if (!errors.some((e) => e.startsWith("Hierarchy: herb_spice") || e.startsWith("Hierarchy: reciprocal"))) {
    passed += 1;
  }
...
function validateByGroup({ byGroup, byId, groups, errors, checks }) {
  ...
  for (const group of groups) {
    const expected = group.herb_spice_ids.sort();
    const indexed = (byGroup[group.slug] ?? []).slice().sort();
    if (JSON.stringify(indexed) !== JSON.stringify(expected)) {
      errors.push(`By group: ${group.slug} index mismatch with group herb_spice_ids`);
    }
  }
...
function validateCrossIndex({ bootstrap, groups, bySlug, byId, byUsageIntensity, byGroup, vocabularyIndex, errors, checks }) {
  let passed = 0;
  const allIds = new Set(Object.keys(byId));

  for (const group of groups) {
    for (const id of group.herb_spice_ids) {
      const ref = byId[id];
      if (!ref) {
        errors.push(`Cross-index: group ${group.slug} herb_spice_id ${id} missing from by_id`);
        continue;
      }
      if (bySlug[ref.slug] !== id) errors.push(`Cross-index: group herb_spice ${id} by_slug mismatch`);
      if (bootstrap.hierarchy.herb_spice_to_group[ref.slug] !== group.slug) {
        errors.push(`Cross-index: group ${group.slug} herb_spice ${ref.slug} hierarchy mismatch`);
      }
    }
  }
...
function validateNamespace({ byNamespace, categories, groups, byId, errors, checks }) {
  ...
  const nsCategory = byNamespace["food.herb"] ?? [];

  if (JSON.stringify(nsCategory) !== JSON.stringify(categoryIds.sort())) {
    errors.push("Namespace: food.herb namespace mismatch with categories");
  } else passed += 1;
  ...
  for (const group of groups) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      errors.push(`Namespace: ${group.id} namespace mismatch with group herb_spice_ids`);
    }
  }
...
const BOTANICAL_SEPARATED = [
  { slug: "cilantro", group: "fresh-herbs", partner: "coriander-seed", partnerGroup: "whole-spices" },
  { slug: "dill", group: "fresh-herbs", partner: "dill-seed", partnerGroup: "whole-spices" },
];

function validateBotanicalOwnership({ byId, bySlug, errors, checks }) {
  let passed = 0;

  for (const rule of BOTANICAL_SEPARATED) {
    const idA = bySlug[rule.slug];
    const idB = bySlug[rule.partner];
    if (!idA) errors.push(`BOTAN-001: missing canonical entity ${rule.slug}`);
    if (!idB) errors.push(`BOTAN-001: missing canonical entity ${rule.partner}`);
    if (idA && idB) {
      if (idA === idB) {
        errors.push(
          `BOTAN-001: ${rule.slug} and ${rule.partner} collapsed to same canonical ID`
        );
      }
      const refA = byId[idA];
      const refB = byId[idB];
      if (refA && refA.parent_group !== rule.group) {
        errors.push(`BOTAN-001: ${rule.slug} must be in ${rule.group}, got ${refA.parent_group}`);
      }
      if (refB && refB.parent_group !== rule.partnerGroup) {
        errors.push(
          `BOTAN-001: ${rule.partner} must be in ${rule.partnerGroup}, got ${refB.parent_group}`
        );
      }
    }
  }

  if (!errors.some((e) => e.startsWith("BOTAN-001"))) passed += 1;
  checks.botanical_ownership = passed;
  return passed;
}
...
function validateDeterminism(loaded, errors, checks) {
  ...
  const rebuilt = compileHerbSpiceRuntime(catalog);
...
function main() {
  ...
  const bootstrap = loaded["herb-spice-bootstrap.json"];
  const categories = loaded["herb-spice-categories.json"];
  const groups = loaded["herb-spice-groups.json"];
  const indexes = loaded["herb-spice-indexes.json"];
  ...
  validateBotanicalOwnership({ byId, bySlug, errors, checks });
  ...
  const report = {
    ...
    metrics: {
      ...
      "Botanical ownership checks": checks.botanical_ownership,
