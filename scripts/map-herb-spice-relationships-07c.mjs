#!/usr/bin/env node
/**
 * FOOD-07C.3 — Structural relationship mapper for herb & spice ontology.
 * Level 1 (structural) + Level 2 (intrinsic similarity) only — no editorial edges.
 *
 * Run: node scripts/map-herb-spice-relationships-07c.mjs
 */
...
import { ARTIFACTS, serializeRuntime } from "./bootstrap-herb-spice-runtime.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const RELATIONSHIPS_PATH = path.join(RUNTIME_DIR, "herb-spice-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/herb-spice-relationship-mapper-report.json");
...
function requireRuntimeValidation() {
  const result = spawnSync("node", ["scripts/validate-runtime-herb-spice-07c.mjs"], {
    cwd: ROOT,
    encoding: "utf8",
...
function loadRuntime() {
  ...
  return {
    bootstrap: loaded["herb-spice-bootstrap.json"],
    categories: loaded["herb-spice-categories.json"],
    groups: loaded["herb-spice-groups.json"],
    indexes: loaded["herb-spice-indexes.json"],
    byId: loaded["herb-spice-indexes.json"].by_id,
  };
}
...
function groupHerbSpicesByField(herbSpices, field) {
  const buckets = {};
  for (const herbSpice of herbSpices) {
    const value = herbSpice[field] ?? "";
    if (value === "" || value === "none") continue;
    if (!buckets[value]) buckets[value] = [];
    buckets[value].push(herbSpice.id);
  }
  return buckets;
}

export function mapHerbSpiceRelationships(runtime) {
  ...
  const herbSpices = Object.values(byId);

  for (const herbSpice of herbSpices) {
    const group = groupBySlug[herbSpice.parent_group];
    if (!group) continue;

    addEdge(edges, seen, makeEdge(herbSpice.id, "belongs_to_group", group.id, "structural"));

    const category = categoryBySlug[group.parent_category];
    if (category) {
      addEdge(edges, seen, makeEdge(herbSpice.id, "belongs_to_category", category.id, "structural"));
    }
  }
  ...
  for (const group of groups) {
    ...
    for (const herbSpiceId of group.herb_spice_ids) {
      addEdge(edges, seen, makeEdge(group.id, "group_contains", herbSpiceId, "structural"));
    }
  }

  for (const [relationship, field] of Object.entries(METADATA_FIELDS)) {
    const buckets = groupHerbSpicesByField(herbSpices, field);
...
function validateRelationships(output, entityIds) {
  ...
  for (const edge of output.edges) {
    ...
    if (FORBIDDEN_EDITORIAL_TYPES.has(edge.relationship)) {
      errors.push(`Editorial relationship forbidden in FOOD-07C: ${edge.relationship}`);
    }
...
const BOTANICAL_SEPARATED_SLUGS = [
  ["cilantro", "coriander-seed"],
  ["dill", "dill-seed"],
];

function validateBotanicalOwnershipRuntime(output, byId, bySlug) {
  const errors = [];

  for (const [slugA, slugB] of BOTANICAL_SEPARATED_SLUGS) {
    const idA = bySlug[slugA];
    const idB = bySlug[slugB];
    if (!idA) errors.push(`BOTAN-001: runtime missing entity slug ${slugA}`);
    if (!idB) errors.push(`BOTAN-001: runtime missing entity slug ${slugB}`);
    if (idA && idB) {
      if (idA === idB) {
        errors.push(`BOTAN-001: runtime collapsed ${slugA} and ${slugB} to one canonical ID`);
      }
      if (!byId[idA] || !byId[idB]) {
        errors.push(`BOTAN-001: runtime by_id missing separated pair ${slugA}/${slugB}`);
      }
    }
  }
  ...
  for (const edge of output.edges) {
    if (edge.source === edge.target) {
      errors.push(`BOTAN-001: self-referential edge ${edge.relationship} on ${edge.source}`);
    }
    if (
      edge.relationship === "shares_scientific_name" &&
      byId[edge.source]?.slug &&
      byId[edge.target]?.slug
    ) {
      const pair = [byId[edge.source].slug, byId[edge.target].slug].sort().join("|");
      const allowedSharedSci = ["cilantro|coriander-seed", "dill|dill-seed"];
      if (allowedSharedSci.includes(pair)) continue;
    }
  }
...
function main() {
  ...
  const output = mapHerbSpiceRelationships(runtime);

  const validation = validateRelationships(output, entityIds);
  const botanicalErrors = validateBotanicalOwnershipRuntime(output, runtime.byId, runtime.indexes.by_slug);
  const allErrors = [...validation.errors, ...botanicalErrors];
  const determinismPass =
    serializeRuntime(output) === serializeRuntime(mapHerbSpiceRelationships(runtime));
  ...
  if (overall === "FAIL") {
    const report = {
      ...
      metrics: {
        ...
        "Botanical ownership violations": botanicalErrors.length,
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/herb-spice-relationships.json",
    metrics: {
      "Relationship types": output.stats.relationship_types,
      "Total edges": output.stats.total_edges,
      "Level 1 structural edges": output.stats.level_1_structural_edges,
