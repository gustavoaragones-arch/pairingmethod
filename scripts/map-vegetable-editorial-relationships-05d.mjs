#!/usr/bin/env node
/**
 * FOOD-05D — Editorial relationship mapper for vegetable ontology.
 ...
 *
 * Run: node scripts/map-vegetable-editorial-relationships-05d.mjs
 */
...
import { EDITORIAL_CURATED, FORWARD_REFERENCE_IDS } from "./vegetable-editorial-seed-05d.js";
...
const OUTPUT_PATH = path.join(RUNTIME_DIR, "vegetable-editorial-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/vegetable-editorial-relationship-report.json");
...
function isCrossDomainTarget(target) {
  return target.startsWith("food.") && !target.startsWith("food.vegetable.");
}

export function mapVegetableEditorialRelationships({ slugMap, catalog }) {
  ...
  for (const entry of EDITORIAL_CURATED) {
    ...
    if (isCrossDomainTarget(entry.target)) {
      ...
      targetId = resolveVegetableId(slugMap, entry.target);
      if (!targetId) {
  ...
  return {
    meta: {
      ...
      inputs: [
        ...
        "scripts/vegetable-editorial-seed-05d.js",
      ],
      scope_exclusions: [
        "wine_pairings",
        "preparation_advice",
...
function validateEditorial(output, structural, vegetableIds) {
  const errors = [];
  const seen = new Set();
  ...
  for (const edge of output.edges) {
    ...
    } else if (!vegetableIds.has(edge.target)) {
      missingEntities += 1;
...
function main() {
  ...
  const output = mapVegetableEditorialRelationships({ slugMap, catalog });
  const validation = validateEditorial(output, structural, output.vegetableIds);
  const rebuilt = mapVegetableEditorialRelationships({ slugMap, catalog });
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/vegetable-editorial-relationships.json",
