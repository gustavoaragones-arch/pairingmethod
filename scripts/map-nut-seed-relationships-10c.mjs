#!/usr/bin/env node
/**
 * FOOD-10C.3 — Structural relationship mapper for nut-seed ontology.
 * Level 1 (structural) + Level 2 (intrinsic similarity) only — no editorial edges.
 *
 * Run: node scripts/map-nut-seed-relationships-10c.mjs
 */
...
import { ARTIFACTS, serializeRuntime } from "./bootstrap-nut-seed-runtime.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const RUNTIME_DIR = path.join(ROOT, "data/runtime");
const RELATIONSHIPS_PATH = path.join(RUNTIME_DIR, "nut-seed-relationships.json");
const REPORT_PATH = path.join(ROOT, "reports/nut-seed-relationship-mapper-report.json");
...
const NUT001_GROUP_ASSIGNMENTS = [
...
function requireRuntimeValidation() {
  const result = spawnSync("node", ["scripts/validate-runtime-nut-seed-10c.mjs"], {
    cwd: ROOT,
    encoding: "utf8",
...
function loadRuntime() {
  ...
  return {
    bootstrap: loaded["nut-seed-bootstrap.json"],
    categories: loaded["nut-seed-categories.json"],
    groups: loaded["nut-seed-groups.json"],
    indexes: loaded["nut-seed-indexes.json"],
    byId: loaded["nut-seed-indexes.json"].by_id,
  };
}
...
export function mapNutSeedRelationships(runtime) {
  ...
  for (const group of groups) {
    ...
    for (const entityId of group.nut_seed_ids) {
...
function validateRelationships(output, entityIds) {
  ...
  for (const edge of output.edges) {
    ...
    if (FORBIDDEN_EDITORIAL_TYPES.has(edge.relationship)) {
      errors.push(`Editorial relationship forbidden in FOOD-10C: ${edge.relationship}`);
    }
...
function validateNutSeedGovernanceRuntime(output, byId, bySlug) {
  const errors = [];
  ...
  for (const { slug, group } of NUT001_GROUP_ASSIGNMENTS) {
    const id = bySlug[slug];
    if (!id) {
      errors.push(`NUT-001: runtime missing entity slug ${slug}`);
      continue;
    }
    const ref = byId[id];
    if (ref && ref.parent_group !== group) {
      errors.push(`NUT-001: ${slug} must be in ${group}, got ${ref.parent_group}`);
    }
  }

  if (bySlug["peanut"] && byId[bySlug["peanut"]]?.parent_group === "tree-nuts") {
    errors.push("BOTAN-001: peanut must not be classified as tree-nuts in runtime");
  }

  for (const [slugA, slugB] of NUT002_SEPARATED_PAIRS) {
    const idA = bySlug[slugA];
    const idB = bySlug[slugB];
    if (!idA) errors.push(`NUT-002: runtime missing entity slug ${slugA}`);
    if (!idB) errors.push(`NUT-002: runtime missing entity slug ${slugB}`);
    if (idA && idB) {
      if (idA === idB) {
        errors.push(`NUT-002: runtime collapsed ${slugA} and ${slugB} to one canonical ID`);
      }
      if (!byId[idA] || !byId[idB]) {
        errors.push(`NUT-002: runtime by_id missing separated pair ${slugA}/${slugB}`);
      }
    }
  }
  ...
  for (const edge of output.edges) {
    if (edge.source === edge.target) {
      errors.push(`Self-referential edge ${edge.relationship} on ${edge.source}`);
    }
  }
...
function main() {
  ...
  const output = mapNutSeedRelationships(runtime);

  const validation = validateRelationships(output, entityIds);
  const governanceErrors = validateNutSeedGovernanceRuntime(
    output,
    runtime.byId,
    runtime.indexes.by_slug
  );
  const allErrors = [...validation.errors, ...governanceErrors];
  const determinismPass =
    serializeRuntime(output) === serializeRuntime(mapNutSeedRelationships(runtime));
  ...
  if (overall === "FAIL") {
    const report = {
      ...
      metrics: {
        ...
        "Governance violations": governanceErrors.length,
  ...
  const report = {
    ...
    overall_result: "PASS",
    errors: [],
    output: "data/runtime/nut-seed-relationships.json",
    metrics: {
      "Relationship types": output.stats.relationship_types,
      "Total edges": output.stats.total_edges,
      "Level 1 structural edges": output.stats.level_1_structural_edges,
