 ...
 *
 * AQ-02B3: group/category catalog entries also carry a distinct
 * `introduction` field — a fuller descriptive paragraph, not a
 * restatement of `summary` (e.g. summary: "Pomes — 12 canonical fruits
 * in the Fruit Ontology."; introduction: "Pomes in the Fruit Ontology —
 * core-bearing fruits used fresh and in cooking, including apple and
 * pear."). It was previously extracted into the narrative pipeline but
 * never rendered anywhere (AQ-01R finding TC-09). It renders here as a
 * second paragraph in the same lead section rather than a separate
 * heading, since it's complementary to summary, not a competing
 * "quick answer" — and is skipped only if byte-identical to summary,
 * so no entity can ever show the same sentence twice.
 */
export function renderEducationalIntroduction(name, narrative, heading = "About This Entry") {
  const text = narrative?.summary;
  if (!text) return "";
  const introduction =
    narrative?.introduction && narrative.introduction !== text
      ? `\n<p>${escapeHtml(narrative.introduction)}</p>`
      : "";
  return `<section class="direct-answer narrative-summary narrative-intro" aria-label="${escapeHtml(heading)}">
<h2>${escapeHtml(heading)}</h2>
<p>${escapeHtml(text)}</p>${introduction}
</section>`;
}
