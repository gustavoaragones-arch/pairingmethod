...
<ul class="term-entity-link-list">${s.typical_regions
  .map((r) => `<li>${escapeHtml(typeof r === "string" ? r : r.name)}</li>`)
  .join("")}
</ul>
</section>`);
  }
...
<ul class="term-entity-pill-list">${ctx.substitutes
  ...
  const servingBits = [
    s.serving_temperature ? `<li><strong>Temperature:</strong> ${escapeHtml(s.serving_temperature)}</li>` : "",
    s.glassware ? `<li><strong>Glassware:</strong> ${escapeHtml(s.glassware)}</li>` : "",
    s.decanting ? `<li><strong>Decanting:</strong> ${escapeHtml(s.decanting)}</li>` : "",
    s.aging ? `<li><strong>Cellaring / aging:</strong> ${escapeHtml(s.aging)}</li>` : "",
  ].filter(Boolean);
