...
export function computeGraphMaturity(taxonomy) {
  ...
  for (const technique of techniques) {
    const edges = buildWinemakingTechniqueGraphEdges(technique, taxonomy);
    totalRelationships += edges.edge_count;
    entityDegrees.push({ kind: "winemaking_technique", slug: technique.slug, degree: edges.edge_count });
  }

  for (const fault of faults) {
    const edges = buildWineFaultGraphEdges(fault, taxonomy);
    totalRelationships += edges.edge_count;
    entityDegrees.push({ kind: "wine_fault", slug: fault.slug, degree: edges.edge_count });
  }
  ...
  const totalEntities =
    descriptors.length + styles.length + regions.length + servings.length + techniques.length + faults.length + grapes.length;

  const withFaq =
    styles.filter((s) => s.faq?.length).length +
    regions.filter((r) => r.faq?.length).length +
    servings.filter((s) => s.faq?.length).length +
    techniques.filter((t) => t.faq?.length).length +
    faults.filter((f) => f.faq?.length).length;
  ...
  return {
    ...
    entities_with_structured_data: styles.length + regions.length + servings.length + techniques.length + faults.length,
...
export function validateGraphEdges(taxonomy) {
  ...
  const techniqueErrors = validateWinemakingTechniqueCatalog(taxonomy);
  for (const msg of techniqueErrors) {
    broken.push(`technique-catalog: ${msg}`);
  }

  const faultErrors = validateWineFaultCatalog(taxonomy);
  for (const msg of faultErrors) {
    broken.push(`fault-catalog: ${msg}`);
  }
