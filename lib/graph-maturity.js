...
export function computeGraphMaturity(taxonomy) {
  ...
  const totalEntities =
    descriptors.length + styles.length + regions.length + servings.length + techniques.length + grapes.length;
  ...
  return {
    ...
    entities_with_structured_data: styles.length + regions.length + servings.length + techniques.length,
