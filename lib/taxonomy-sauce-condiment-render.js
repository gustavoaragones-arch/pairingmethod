...
const LEAF_LINK_SECTIONS = [
  ...
  ["classic_wine_pairings", "Classic wine pairings"],
  ["contrasting_wine_pairings", "Contrasting wine pairings"],
  ["regional_wine_pairings", "Regional wine pairings"],
...
function renderDefinitionList(entries) {
  ...
  const rows = entries
    ...
    .join("");
  return `<dl class="sauce-condiment-characteristics-list">${rows}</dl>`;
}
...
function renderCharacteristicsBlock(title, characteristics) {
  ...
  return `<section class="term-entity-section sauce-condiment-characteristics" aria-labelledby="${headingId}">
...
export function renderLinkSection(title, links) {
  ...
  const headingId = `${slugifyHeading(title)}-heading`;
  return `<section class="term-entity-section sauce-condiment-link-section" aria-labelledby="${headingId}">
<h2 id="${headingId}">${escapeHtml(title)}</h2>
...
<h1>${escapeHtml(overview.display_name ?? page.identity.title)}</h1>
  ...
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section sauce-condiment-overview" aria-labelledby="overview-heading">
...
${overviewHtml}
  ...
  sections.push(renderLinkSection("Member sauces & condiments", sectionsNav.member_sauce_condiments));
