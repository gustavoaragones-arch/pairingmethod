...
// AQ-04A finding: buildFoodWebPageSchema, buildFoodDefinedTermSchema,
// buildFoodSchemas, buildGroupCollectionPageSchema,
// buildGroupDefinedTermSetSchema, buildGroupSchemas,
// buildCategoryCollectionPageSchema, buildCategoryDefinedTermSetSchema, and
// buildCategorySchemas below are dead code — lib/food-publication/schema.js
// (the only live schema generator) defines and uses its own local, domain-
// aware equivalents instead of importing these. Left in place for now
// (removal is a code-hygiene change, out of scope for a semantic-integrity
// phase) but do not call these directly: they predate the AQ-04B domain-
// aware Taxon gating and would misrepresent cheese pages if used.
...
/**
 * AQ-04B — `scientific_name` denotes the entity's own taxonomic identity in
 * every domain except cheese, where docs/CHEESE_GOVERNANCE.md §5 documents
 * it as milk-source species only. Asserting `@type: Taxon` about a cheese
 * page's own subject would misrepresent that page as being about the milk
 * animal's species, not the cheese. `ownIdentity` gates that assertion;
 * callers that omit it default to true (the 10-domain case) to avoid
 * silently breaking existing call sites.
 */
export function buildFoodTaxonSchema(page, ids, { ownIdentity = true } = {}) {
  if (!page.overview.scientific_name || !ownIdentity) return null;
