/**
 * FOOD-06F — Fungi HTML section renderers.
 ...
 * taxonomy-render.js — this file only supplies fungi-specific config.
 */
...
import { renderWhyTheseWinesWork } from "./fungi-wine-pairing-explanation.js";
...
export function renderFungiSections(page, linkSet) {
  const sections = [];
  ...
  sections.push(`<header class="term-entity-hero fungi-hero">
<p class="term-entity-label">Fungus</p>
<h1>${escapeHtml(name)}</h1>
  ...
  sections.push(renderWhyTheseWinesWork(page.identity?.id, name, linkSet));
  ...
  const classificationHtml = renderOverviewRows(classificationRows, "fungi-characteristics-list");
  if (classificationHtml) {
    sections.push(`<section class="term-entity-section fungi-overview" aria-labelledby="scientific-classification-heading">
...
${classificationHtml}
  ...
  sections.push(
    renderCharacteristicsBlock(
      ...
      "fungi-characteristics-list"
    )
  );

  for (const [key, title] of RELATED_INGREDIENT_SECTIONS) {
    sections.push(renderMetaLinkSection(title, sectionsNav[key], "fungi-link-section"));
  }
...
export function renderFungiGroupSections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero fungi-group-hero">
<p class="term-entity-label">Fungi Group</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
  ...
  const overviewHtml = renderOverviewRows(overviewRows, "fungi-characteristics-list");
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section fungi-overview" aria-labelledby="scientific-classification-heading">
<h2 id="scientific-classification-heading">Scientific Classification</h2>
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Member fungi", sectionsNav.member_fungi, "fungi-link-section"));
  sections.push(renderMetaLinkSection("Related groups", sectionsNav.related_groups, "fungi-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderFungiCategorySections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero fungi-category-hero">
<p class="term-entity-label">Fungi Category</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
...
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Groups", sectionsNav.groups, "fungi-link-section"));
  sections.push(renderMetaLinkSection("Related categories", sectionsNav.related_categories, "fungi-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderFungiBreadcrumb(page) {
  return renderBreadcrumb(page.breadcrumbs ?? []);
}

export function assembleFungiEntityPage({
  template,
  page,
  bodySectionsHtml,
  jsonLd,
  ...
  return {
    ...
    BREADCRUMB: renderFungiBreadcrumb(page),
