...
function renderDefinitionList(entries) {
  ...
  const rows = entries
    ...
    .join("");
  return `<dl class="fungi-characteristics-list">${rows}</dl>`;
}
...
function renderCharacteristicsBlock(title, characteristics) {
  ...
  return `<section class="term-entity-section fungi-characteristics" aria-labelledby="${headingId}">
...
export function renderLinkSection(title, links) {
  ...
  const headingId = `${slugifyHeading(title)}-heading`;
  return `<section class="term-entity-section fungi-link-section" aria-labelledby="${headingId}">
<h2 id="${headingId}">${escapeHtml(title)}</h2>
...
<h1>${escapeHtml(overview.display_name ?? page.identity.title)}</h1>
  ...
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section fungi-overview" aria-labelledby="overview-heading">
...
<h1>${escapeHtml(overview.display_name ?? page.identity.title)}</h1>
  ...
  const overviewRows = [
    ...
    ["Fungi count", overview.food_count != null ? String(overview.food_count) : null],
  ];
...
${overviewHtml}
  ...
  sections.push(renderLinkSection("Member fungi", sectionsNav.member_fungi));
