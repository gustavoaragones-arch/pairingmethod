...
import { normalizeEvidence, resolveReasonEntities, summarizeRelationshipEvidence } from "./relationship-evidence.js";
import { CONFIDENCE_LEVELS } from "./relationship-evidence-types.js";
...
/**
 * @typedef {{ type: string, target: string, target_kind: string, source_field?: string, inferred?: boolean, evidence?: { reason?: Array<string|{ kind?: string, slug: string, text?: string }>, confidence?: string, notes?: string } }} TypedEdge
 */

export { normalizeEvidence, resolveReasonEntities, summarizeRelationshipEvidence, CONFIDENCE_LEVELS };

/**
 * Attach optional normalized evidence to a typed edge (backward compatible).
 *
 * @param {TypedEdge} edge
 * @param {unknown} evidence
 */
export function attachEvidenceToEdge(edge, evidence) {
  const normalized = normalizeEvidence(evidence);
  if (!normalized) return edge;
  return { ...edge, evidence: normalized };
}
