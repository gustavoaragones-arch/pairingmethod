...
export function buildSemanticGraph(options = {}) {
  ...
  const explicit = applyRelationshipEvidence(
    collectAllTypedEdges({
      taxonomy,
      regionReverse,
      servingReverse,
      knowledgeGraph,
  ...
  const evidenceStats = computeEvidenceStats(explicit);
  const validationErrors = [
    ...validateSemanticEdges(all.filter((e) => !e.inferred)),
    ...validateRelationshipEvidence(all, { taxonomy }),
...
/**
 * Benchmark evidence-enriched relationship lookup.
 */
export function benchmarkEvidenceTraversal(graph, iterations = 3000) {
  const edgesWithEvidence = graph.explicit_edges.filter((e) => e.evidence);
  if (!edgesWithEvidence.length) {
    return { iterations: 0, ms_per_lookup: 0, edges_tested: 0 };
  }

  const start = performance.now();
  for (let i = 0; i < iterations; i += 1) {
    const edge = edgesWithEvidence[i % edgesWithEvidence.length];
    outboundNeighbors(graph, edge.source_kind, edge.source, edge.type);
    edge.evidence?.reason?.length;
  }
  const elapsed = performance.now() - start;
  return {
    iterations,
    ms_total: Number(elapsed.toFixed(3)),
    ms_per_lookup: Number((elapsed / iterations).toFixed(5)),
    edges_tested: edgesWithEvidence.length,
  };
}
