/**
 * FOOD-10F — Nut & Seed HTML section renderers.
 ...
 * taxonomy-render.js — this file only supplies nut/seed-specific config.
 */
...
export function renderNutSeedSections(page, linkSet) {
  const sections = [];
  ...
  sections.push(`<header class="term-entity-hero nut-seed-hero">
<p class="term-entity-label">Nut & Seed</p>
<h1>${escapeHtml(name)}</h1>
  ...
  sections.push(renderWhyTheseWinesWork("nut-seed", page.identity?.id, name, linkSet));
  ...
  const classificationHtml = renderOverviewRows(classificationRows, "nut-seed-characteristics-list");
  if (classificationHtml) {
    sections.push(`<section class="term-entity-section nut-seed-overview" aria-labelledby="scientific-classification-heading">
...
${classificationHtml}
  ...
  sections.push(
    renderCharacteristicsBlock(
      ...
      characteristics,
      TAXONOMY_LABELS,
      "nut-seed-characteristics",
      "nut-seed-characteristics-list"
    )
  );

  for (const [key, title] of RELATED_INGREDIENT_SECTIONS) {
    sections.push(renderMetaLinkSection(title, sectionsNav[key], "nut-seed-link-section"));
  }
...
export function renderNutSeedGroupSections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero nut-seed-group-hero">
<p class="term-entity-label">Nut & Seed Group</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
  ...
  const overviewHtml = renderOverviewRows(overviewRows, "nut-seed-characteristics-list");
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section nut-seed-overview" aria-labelledby="scientific-classification-heading">
<h2 id="scientific-classification-heading">Scientific Classification</h2>
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Member nuts & seeds", sectionsNav.member_nuts_seeds, "nut-seed-link-section"));
  sections.push(renderMetaLinkSection("Related groups", sectionsNav.related_groups, "nut-seed-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderNutSeedCategorySections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero nut-seed-category-hero">
<p class="term-entity-label">Nut & Seed Category</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
...
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Groups", sectionsNav.groups, "nut-seed-link-section"));
  sections.push(renderMetaLinkSection("Related categories", sectionsNav.related_categories, "nut-seed-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderNutSeedBreadcrumb(page) {
  return renderBreadcrumb(page.breadcrumbs ?? []);
}

export function assembleNutSeedEntityPage({
  template,
  page,
  bodySectionsHtml,
  jsonLd,
  ...
  return {
    ...
    BREADCRUMB: renderNutSeedBreadcrumb(page),
