...
import { inferEntityType, SUPPORTED_ENTITY_TYPES } from "./entity-model.js";
...
/** @deprecated Use SUPPORTED_ENTITY_TYPES from entity-model.js */
export const SUPPORTED_NODE_TYPES = Object.freeze([
  "category",
  "group",
  "descriptor",
  "entity",
  ...SUPPORTED_ENTITY_TYPES.filter((t) => !["descriptor_category", "descriptor_group", "descriptor"].includes(t)),
]);
...
export function compactTaxonomyNodeRecord(taxonomy, slug) {
  ...
  if (node.type === "descriptor") {
    return {
      ...
      name: node.name,
      type: node.type,
      entityType: node.entity_type ?? inferEntityType(node),
      domain: node.domain,
