...
function checkEntityIdentityLanguageIndependent(root) {
  ...
  //       resolve to a real catalog record? This is NOT checkable yet
  //       because LANG-01 is schema-only by design (entries: []). It is
  //       explicitly deferred to LANG-02+, not silently skipped.
  ...
  const populationValidationStatus =
    entriesCount === 0
      ? "deferred_schema_only"
      : "would_require_live_entity_id_cross_reference_against_catalogs";

  return {
    id: "V7_entity_identity_language_independent",
    description:
      "Schema contract requires entity_id to reference an existing ontology id (checked now). Populated-entry cross-reference validation is a SEPARATE, deferred concern — LANG-01 is schema-only (entries: []), so there is nothing yet to cross-reference; this PASSES on the schema contract alone and explicitly states population validation is deferred, rather than implying entries were checked.",
    pass: schemaContractValid && entriesCount === 0,
    evidence: {
      schema_contract_valid: schemaContractValid,
      entries_count: entriesCount,
      population_validation_status: populationValidationStatus,
      population_validation_note: "LANG-02+ MUST validate each populated entry's entity_id against the live catalogs before this check can certify populated data — that validation does not exist yet and this check does not claim it does.",
    },
  };
}
