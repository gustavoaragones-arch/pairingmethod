/**
 * FOOD-07F — Herb & Spice HTML section renderers.
 ...
 * taxonomy-render.js — this file only supplies herb/spice-specific config.
 */
...
export function renderHerbSpiceSections(page, linkSet) {
  const sections = [];
  ...
  sections.push(`<header class="term-entity-hero herb-spice-hero">
<p class="term-entity-label">Herb or Spice</p>
<h1>${escapeHtml(name)}</h1>
  ...
  sections.push(renderWhyTheseWinesWork("herb-spice", page.identity?.id, name, linkSet));
  ...
  const classificationHtml = renderOverviewRows(classificationRows, "herb-spice-characteristics-list");
  if (classificationHtml) {
    sections.push(`<section class="term-entity-section herb-spice-overview" aria-labelledby="scientific-classification-heading">
...
${classificationHtml}
  ...
  sections.push(
    renderCharacteristicsBlock(
      ...
      characteristics,
      TAXONOMY_LABELS,
      "herb-spice-characteristics",
      "herb-spice-characteristics-list"
    )
  );

  for (const [key, title] of RELATED_INGREDIENT_SECTIONS) {
    sections.push(renderMetaLinkSection(title, sectionsNav[key], "herb-spice-link-section"));
  }
...
export function renderHerbSpiceGroupSections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero herb-spice-group-hero">
<p class="term-entity-label">Herb & Spice Group</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
  ...
  const overviewHtml = renderOverviewRows(overviewRows, "herb-spice-characteristics-list");
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section herb-spice-overview" aria-labelledby="scientific-classification-heading">
<h2 id="scientific-classification-heading">Scientific Classification</h2>
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Member herbs & spices", sectionsNav.member_herb_spices, "herb-spice-link-section"));
  sections.push(renderMetaLinkSection("Related groups", sectionsNav.related_groups, "herb-spice-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderHerbSpiceCategorySections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero herb-spice-category-hero">
<p class="term-entity-label">Herb & Spice Category</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
...
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Groups", sectionsNav.groups, "herb-spice-link-section"));
  sections.push(renderMetaLinkSection("Related categories", sectionsNav.related_categories, "herb-spice-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderHerbSpiceBreadcrumb(page) {
  return renderBreadcrumb(page.breadcrumbs ?? []);
}

export function assembleHerbSpiceEntityPage({
  template,
  page,
  bodySectionsHtml,
  jsonLd,
  ...
  return {
    ...
    BREADCRUMB: renderHerbSpiceBreadcrumb(page),
