...
function renderDefinitionList(entries) {
  ...
  const rows = entries
    ...
    .join("");
  return `<dl class="herb-spice-characteristics-list">${rows}</dl>`;
}
...
function renderCharacteristicsBlock(title, characteristics) {
  ...
  return `<section class="term-entity-section herb-spice-characteristics" aria-labelledby="${headingId}">
...
export function renderLinkSection(title, links) {
  ...
  const headingId = `${slugifyHeading(title)}-heading`;
  return `<section class="term-entity-section herb-spice-link-section" aria-labelledby="${headingId}">
<h2 id="${headingId}">${escapeHtml(title)}</h2>
...
<h1>${escapeHtml(overview.display_name ?? page.identity.title)}</h1>
  ...
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section herb-spice-overview" aria-labelledby="overview-heading">
...
<h1>${escapeHtml(overview.display_name ?? page.identity.title)}</h1>
  ...
  const overviewRows = [
    ...
    ["Herb & spice count", overview.food_count != null ? String(overview.food_count) : null],
  ];
...
${overviewHtml}
  ...
  sections.push(renderLinkSection("Member herbs & spices", sectionsNav.member_herb_spices));
