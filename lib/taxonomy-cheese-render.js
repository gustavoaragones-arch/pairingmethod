...
function renderDefinitionList(entries) {
  ...
  const rows = entries
    ...
    .join("");
  return `<dl class="cheese-characteristics-list">${rows}</dl>`;
}
...
function renderCharacteristicsBlock(title, characteristics) {
  ...
  return `<section class="term-entity-section cheese-characteristics" aria-labelledby="${headingId}">
...
export function renderLinkSection(title, links) {
  ...
  const headingId = `${slugifyHeading(title)}-heading`;
  return `<section class="term-entity-section cheese-link-section" aria-labelledby="${headingId}">
<h2 id="${headingId}">${escapeHtml(title)}</h2>
...
<h1>${escapeHtml(overview.display_name ?? page.identity.title)}</h1>
  ...
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section cheese-overview" aria-labelledby="overview-heading">
...
${overviewHtml}
  ...
  sections.push(renderLinkSection("Member cheeses", sectionsNav.member_cheeses));
