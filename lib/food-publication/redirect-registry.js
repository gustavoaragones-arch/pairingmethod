...
export function runRedirectRegistryStage(root) {
  ...
  const trailingSlashLines = redirects.map((r) => `${r.from} ${r.to} 301`);
  const nonTrailingSlashLines = redirects
    .filter((r) => r.from.endsWith("/"))
    .map((r) => `${r.from.slice(0, -1)} ${r.to} 301`);
  const lines = [...trailingSlashLines, ...nonTrailingSlashLines];
  const block = `\n${MARKER_START}\n${lines.join("\n")}\n${MARKER_END}\n`;
