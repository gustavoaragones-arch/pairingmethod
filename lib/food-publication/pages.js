...
function leafMetadata(domain, projection, canonicalPath) {
  ...
  const templateDescription = [
    ...
    .filter(Boolean)
    .join(" ");
  // AQ-04B: seo_description is an authored, governed catalog field (100%
  // coverage across all 11 domains) that the generic template above was
  // silently ignoring. Prefer it; keep the template as a fallback for the
  // hypothetical case a future domain hasn't authored it yet.
  const description = projection.narrative?.seo_description ?? templateDescription;
...
function groupMetadata(domain, projection, canonicalPath) {
  ...
  const templateDescription = `${projection.identity.name} — ${projection.food_count} ${domain.entityLabels.leafCount} in the ${domain.metadata.ontologyName}.`;
  const description = projection.narrative?.seo_description ?? templateDescription;
  return {
    title,
    description,
...
function categoryMetadata(domain, projection, canonicalPath) {
  ...
  const templateDescription = `${projection.identity.name} — ${projection.group_count} groups and ${projection.food_count} ${domain.entityLabels.leafCount}.`;
  const description = projection.narrative?.seo_description ?? templateDescription;
  return {
    title,
    description,
