...
const LEGACY_WINE_SECTION_KEYS = new Set([
  "primary_wine_styles",
...
function wineSectionKeys(domain) {
  const keys = new Set(Object.values(domain.wine.byRelationship));
  keys.add("wine_descriptors");
  keys.add("wine_techniques");
  for (const key of LEGACY_WINE_SECTION_KEYS) keys.add(key);
  return keys;
}
...
function leafEditorialSectionKeys(domain) {
  const wineKeys = wineSectionKeys(domain);
  return Object.keys(domain.linkSections.leaf).filter((key) => !wineKeys.has(key));
}
...
function buildWinePairing(domain, linkSet) {
  const sections = linkSet?.sections ?? {};
  const styleRelationships = new Set(
    domain.wine.styleRelationships ?? ["pairs_with_style", "also_pairs_with_style"]
  );
  const wineStyles = [];

  for (const [relationship, bucket] of Object.entries(domain.wine.byRelationship)) {
    if (styleRelationships.has(relationship)) {
      wineStyles.push(...(sections[bucket] ?? []));
    }
  }
