...
function wineStyleRelationships(domain) {
  return domain.wine.styleRelationships ?? ["pairs_with_style", "also_pairs_with_style"];
}

function wineLinkFromEdge(edge, wineRelationshipSections, domain) {
  const sectionKey = wineRelationshipSections.get(edge.relationship);
  if (!sectionKey) return null;

  if (wineStyleRelationships(domain).includes(edge.relationship)) {
    return {
      sectionKey,
...
function buildLeafLinkSet(page, registry, structuralIdx, editorialIdx, pairingIdx, domain) {
  ...
  for (const edge of pairingIdx.bySource.get(leafId) ?? []) {
    const resolved = wineLinkFromEdge(edge, wineRelationshipSections, domain);
