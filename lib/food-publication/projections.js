...
import { loadProteinMigrationPublicationContext, collectEdgesForLeaf, foodEndpointIdsForLeaf } from "../publication/proteinMigrationPublication.js";
...
export function generateProjections(domain, inputs) {
  ...
  const proteinMigration =
    domain.id === "protein" ? loadProteinMigrationPublicationContext(domain.root) : null;
  ...
  const leafProjections = leaves
    .map((leaf) => {
      ...
      const editorialEdges = proteinMigration
        ? collectEdgesForLeaf(leaf.id, editorialBySource, proteinMigration.resolver)
        : editorialBySource[leaf.id] ?? [];

      for (const edge of editorialEdges) {
      ...
      for (const relationship of domain.editorial.skipInMainLoop) {
        ...
        const handler = symmetricHandlers[relationship] ?? {};
        const endpointIds = proteinMigration
          ? foodEndpointIdsForLeaf(leaf.id, proteinMigration.resolver)
          : [leaf.id];
        for (const id of endpointIds) {
          for (const edge of handler[id] ?? []) {
            if (bucket) editorialContext[bucket].push(relationshipRef(edge));
          }
        }
      }

      const pairingEdges = proteinMigration
        ? collectEdgesForLeaf(leaf.id, pairingBySource, proteinMigration.resolver)
        : pairingBySource[leaf.id] ?? [];
      ...
      return {
        ...
        narrative: extractNarrative(leaf, "leaf"),
      };
    })
  ...
  const groupProjections = groups
    .map((group) => {
      ...
      return {
        ...
        ...aggregates,
        narrative: extractNarrative(group, "group"),
      };
    })
    .sort((a, b) => a.identity.id.localeCompare(b.identity.id));
  ...
  const categoryProjections = categories
    .map((category) => {
      ...
      return {
        ...
        narrative: extractNarrative(category, "category"),
      };
    })
    .sort((a, b) => a.identity.id.localeCompare(b.identity.id));
...
function validateProjections(domain, generated) {
  ...
  const proteinMigration =
    domain.id === "protein" ? loadProteinMigrationPublicationContext(domain.root) : null;
  const symmetricSet = new Set(domain.editorial.symmetric);
  ...
  for (const leaf of leafProjections) {
    ...
    for (const ref of allRefs) {
      const sourceCandidates = proteinMigration
        ? foodEndpointIdsForLeaf(leaf.identity.id, proteinMigration.resolver)
        : [leaf.identity.id];
      const targetCandidates = [ref.target];
      if (proteinMigration && ref.target.startsWith("food.")) {
        targetCandidates.push(proteinMigration.resolver.resolveProteinId(ref.target));
      }

      let registryMatch = false;
      for (const source of sourceCandidates) {
        for (const target of [...new Set(targetCandidates)]) {
          const key = canonicalEdgeKey(
            { source, relationship: ref.relationship, target },
            symmetricSet
          );
      ...
      if (!registryMatch) {
        errors.push(`${leaf.identity.id}: missing relationship ${ref.relationship}\t${ref.target}`);
      }
      ...
      if (prepPrefix && ref.relationship === "commonly_prepared_as") {
        ...
        if (
          !entitySets.leafIds.has(ref.target) &&
          !(proteinMigration?.canonicalIds.has(ref.target))
        ) {
