...
import {
  isValidProteinPublicationFoodEndpoint,
...
function loadLinkInputs(domain, pages, projections) {
  const runtime = loadDomainInputs(domain);
  return {
...
function certifyRelationshipCoverage(domain, catalog, projections, state) {
  ...
  const editorial = runtime.editorial;
  ...
  for (const edge of editorial.edges) {
    if (proteinMigration) {
      ...
      if (
        ...
        !domain.editorial.crossDomainTargets.includes(edge.relationship)
      ) {
  ...
  for (const projection of projections.leaf.projections) {
    ...
    for (const ref of refs) {
      const sourceCandidates = proteinMigration
        ? foodEndpointIdsForLeaf(projection.identity.id, proteinMigration.resolver)
      ...
      for (const source of sourceCandidates) {
        for (const target of [...new Set(targetCandidates)]) {
          const key = canonicalEdgeKey(source, ref.relationship, target, symmetric);
      ...
      if (!registryMatch) {
        errors.push(
          `Projection missing relationship edge: ${projection.identity.id}\t${ref.relationship}\t${ref.target}`
        );
      }
    }
  }
