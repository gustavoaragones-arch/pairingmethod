...
function validateSitemaps(deployments, errors) {
  ...
  for (const deployment of deployments) {
    ...
    const expectedSitemapTotal =
      deployment.expectedSitemapCounts?.total ?? deployment.expectedHtmlCounts.total;

    if (domainLocs.length !== expectedSitemapTotal) {
      errors.push(
        `${deployment.id} sitemap URL count ${domainLocs.length} != ${expectedSitemapTotal}`
      );
    }
  }
