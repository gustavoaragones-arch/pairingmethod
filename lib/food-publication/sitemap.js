...
function loadRedirectRequiredLeafSlugs(domain) {
  if (domain.id !== "protein") return new Set();

  const migrationMap = readJson(path.join(domain.root, "data/protein-migration-map.json"));
  const prefix = domain.urls.leafPrefix;
  return new Set(
    migrationMap.migrations
      ...
      .map((entry) => {
        ...
        if (!source.startsWith(prefix) || !source.endsWith("/")) {
          throw new Error(`Invalid Protein migration source path: ${source}`);
        }
...
function filterSitemapCatalog(domain, catalog) {
  const excludedLeafSlugs = loadRedirectRequiredLeafSlugs(domain);
  const leaf = catalog.leaf.filter((page) => !excludedLeafSlugs.has(page.slug));
  return {
    leaf,
    group: catalog.group,
    category: catalog.category,
    all: [...leaf, ...catalog.group, ...catalog.category],
  };
}
...
function generateSitemapArtifacts(domain, lastmod) {
  ...
  const sitemapCatalog = filterSitemapCatalog(domain, catalog);
  ...
  for (const child of childConfigs) {
    const pages = child.pages(sitemapCatalog);
...
function validateSitemapArtifacts(domain, generated) {
  ...
  const groups = [
    { name: "leaf", pages: generated.sitemapCatalog.leaf, entries: generated.entryGroups.leaf },
    { name: "group", pages: generated.sitemapCatalog.group, entries: generated.entryGroups.group },
    {
      name: "category",
      pages: generated.sitemapCatalog.category,
  ...
  for (const page of generated.sitemapCatalog.all) {
    const html = fs.readFileSync(page.filePath, "utf8");
    const canonical = extractCanonicalUrl(html);
