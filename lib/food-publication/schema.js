...
// AQ-04B — domains where `scientific_name` denotes the entity's own
// taxonomic identity (accurate as schema.org Taxon) vs. domains where it
// denotes something else entirely. Cheese is the sole documented exception
// (docs/CHEESE_GOVERNANCE.md §5: scientific_name is milk-source species,
// never cheese identity) — same set used by AQ-02B4's knowledge-integrity
// check and AQ-03A's scientific-identity verifier for the same reason.
const DERIVED_PRODUCT_DOMAINS = new Set(["cheese"]);

function scientificNameIsOwnIdentity(domain) {
  return !DERIVED_PRODUCT_DOMAINS.has(domain.id);
}
...
function buildFoodWebPageSchema(domain, page, ids) {
  ...
  if (page.overview.scientific_name) {
    if (scientificNameIsOwnIdentity(domain)) {
      about.push({ "@type": "Taxon", "@id": ids.taxon, name: page.overview.scientific_name });
    } else {
      about.push({
        "@type": "PropertyValue",
        name: "Milk source species",
        value: page.overview.scientific_name,
      });
    }
  }
...
function buildFoodSchemas(domain, page) {
  ...
  const taxon = buildFoodTaxonSchema(page, ids, { ownIdentity: scientificNameIsOwnIdentity(domain) });
