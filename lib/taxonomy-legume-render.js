/**
 * FOOD-11F — Legume HTML section renderers.
 ...
 * taxonomy-render.js — this file only supplies legume-specific config.
 */
...
const RELATED_INGREDIENT_SECTIONS = [["similar_legumes", "Similar legumes"]];
...
export function renderLegumeSections(page, linkSet) {
  const sections = [];
  ...
  sections.push(`<header class="term-entity-hero legume-hero">
<p class="term-entity-label">Legume</p>
<h1>${escapeHtml(name)}</h1>
  ...
  sections.push(renderWhyTheseWinesWork("legume", page.identity?.id, name, linkSet));
  ...
  const classificationHtml = renderOverviewRows(classificationRows, "legume-characteristics-list");
  if (classificationHtml) {
    sections.push(`<section class="term-entity-section legume-overview" aria-labelledby="scientific-classification-heading">
...
${classificationHtml}
  ...
  sections.push(
    renderCharacteristicsBlock(
      ...
      "legume-characteristics-list"
    )
  );

  for (const [key, title] of RELATED_INGREDIENT_SECTIONS) {
    sections.push(renderMetaLinkSection(title, sectionsNav[key], "legume-link-section"));
  }
...
export function renderLegumeGroupSections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero legume-group-hero">
<p class="term-entity-label">Legume Group</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
  ...
  sections.push(
    renderKnowledgeCards([
      ...
      { label: "Legume count", value: overview.food_count != null ? String(overview.food_count) : null },
    ])
  );
  ...
  const overviewRows = [
    ...
    ["Legume count", overview.food_count != null ? String(overview.food_count) : null],
  ];
  const overviewHtml = renderOverviewRows(overviewRows, "legume-characteristics-list");
  if (overviewHtml) {
    sections.push(`<section class="term-entity-section legume-overview" aria-labelledby="scientific-classification-heading">
<h2 id="scientific-classification-heading">Scientific Classification</h2>
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Member legumes", sectionsNav.member_legumes, "legume-link-section"));
  sections.push(renderMetaLinkSection("Related groups", sectionsNav.related_groups, "legume-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderLegumeCategorySections(page, linkSet) {
  const sections = [];
  const overview = page.overview ?? {};
  const narrative = page.narrative ?? {};
  const sectionsNav = linkSet?.sections ?? {};
  ...
  sections.push(`<header class="term-entity-hero legume-category-hero">
<p class="term-entity-label">Legume Category</p>
<h1>${escapeHtml(name)}</h1>
</header>`);
...
${overviewHtml}
  ...
  sections.push(renderMetaLinkSection("Groups", sectionsNav.groups, "legume-link-section"));
  sections.push(renderMetaLinkSection("Related categories", sectionsNav.related_categories, "legume-link-section"));

  return sections.filter(Boolean).join("\n");
}

export function renderLegumeBreadcrumb(page) {
  return renderBreadcrumb(page.breadcrumbs ?? []);
}

export function assembleLegumeEntityPage({
  template,
  page,
  bodySectionsHtml,
  jsonLd,
  ...
  return {
    ...
    BREADCRUMB: renderLegumeBreadcrumb(page),
